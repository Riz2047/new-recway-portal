...
@section('title')
    {{ __('Verify Your Identity') }} | {{ config('app.name') }}
@endsection
...
<div>
    ...
    <form
        method="POST"
        action="{{ route('customer.locked.verify-mfa.submit') }}"
        x-data="otpForm()"
            ...
            <div class="flex items-center justify-center gap-2 sm:gap-3">
                @for ($i = 0; $i < 6; $i++)
                    <input
                        ...
                        class="otp-box h-12 w-10 rounded-lg border-2 border-gray-300 bg-white text-center text-xl font-bold
    ...
    <div class="mt-5 text-center">
        <form method="POST" action="{{ route('customer.locked.send-mfa') }}" class="inline">
            @csrf
