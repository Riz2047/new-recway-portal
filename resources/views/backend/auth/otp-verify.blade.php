        ...
        <div x-data="resendTimer({{ $cooldownRemaining }})">
            ...
            <template x-if="remaining === 0">
                <form method="POST" action="{{ route($panel . '.otp.resend') }}">
                    @csrf
                    <button type="submit"
                        class="text-sm font-medium text-indigo-600 hover:text-indigo-800 hover:underline dark:text-indigo-400 dark:hover:text-indigo-300">
...
function otpForm() {
    return {
        ...
        onKey(e) {
            if (e.key === 'Backspace') {
                if (!e.target.value && e.target.previousElementSibling?.classList.contains('otp-box')) {
                    e.target.previousElementSibling.focus();
                    e.target.previousElementSibling.value = '';
                    this.syncCode();
                }
            }
            if (e.key === 'ArrowLeft' && e.target.previousElementSibling?.classList.contains('otp-box')) {
                e.target.previousElementSibling.focus();
            }
            if (e.key === 'ArrowRight' && e.target.nextElementSibling?.classList.contains('otp-box')) {
                e.target.nextElementSibling.focus();
            }
        },
        ...
        onPaste(e) {
            ...
            if (pasted.length >= 6) {
                this.$nextTick(() => {
                    if (this.code.length === 6) this.$el.submit();
                });
            }
        },

        syncCode() {
            this.code = [...this.boxes()].map(b => b.value).join('');
        },
    };
}
