<div class="py-6 space-y-4" x-data="{ activeLanguage: 'sv' }">
    <div class="rounded-xl border border-gray-200 bg-white p-4">
        <label class="mb-1.5 block text-sm font-medium text-gray-700">{{ __('Background Check Service') }}</label>
        <select wire:model.live="selectedService"
            ...
            @empty
                <option value="">{{ __('No background check services assigned to this customer') }}</option>
            @endforelse
        </select>
        ...
        @foreach(['sv' => __('Swedish'), 'en' => __('English')] as $lang => $languageLabel)
            <div x-show="activeLanguage === '{{ $lang }}'" x-cloak class="space-y-3">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-base font-semibold text-gray-900">{{ __('Customer Template') }} - {{ $languageLabel }}</h3>
                        <p class="text-xs {{ $isOverridden[$lang] ? 'text-indigo-700' : 'text-gray-500' }}">
                            {{ $isOverridden[$lang] ? __('Customer override is active') : __('Using global template currently') }}
                        </p>
                    </div>
                </div>
