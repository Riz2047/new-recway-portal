...
class PostDatatable extends Datatable
{
    ...
    {
        $status = $post->status ?? '';
        $html = "<span class='" . get_post_status_class($status) . "'>" . ($status ? ucfirst($status) : '-') . "</span>";
    ...
    {
        return $post->categories->pluck('name')->map(fn ($name) => "<span class='badge'>" . ($name ? ucfirst($name) : '') . "</span>")->join(' ');
    }
}
