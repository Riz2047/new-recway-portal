...
class PermissionService
{
    ...
    {
        // Cache permission groups to avoid repeated queries
        return cache()->remember('permission_groups.all', 3600, function () {
            $groups = Permission::select('group_name as name')
                ->groupBy('group_name')
                ->orderBy('group_name')
                ->get();

            // Eager load all permissions at once to avoid N+1
            $allPermissions = Permission::orderBy('group_name')->orderBy('name')->get();
            $permissionsByGroup = $allPermissions->groupBy('group_name');

            // Add the permissions to each group.
            foreach ($groups as $group) {
                $group->setAttribute('permissions', $permissionsByGroup->get($group->name, collect()));
            }
    ...
    public function createPermissions(): array
    {
        ...
        // Clear permission cache after creating/updating permissions
