                                                ...
                                                <?php if (! empty($customer->timra_report)) { ?>
                                                    <button type="button" class="btn btn-sm btn-info text-white p-2"
                                                        onclick="pdf_gene_timra(<?= $_GET['id'] ?>)"><i
                                                            class="bi bi-cloud-download-fill me-2"></i>Generate Referens Template</button>
                                                <?php } ?>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
...
?>
                        <button class="tablinks2 f-14 w-700 text-left"
                            ...
                            style="width: 100% !important;"><span class="bk_text <?= $color ?>">BK for criminal,
                                economy & social media</span> <i class="float-right bx bxs-chevron-down arrow"
...
<script type="text/template" id="commentTemplate">
    <div class="mt-2 bg-light p-2 comment">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <p class="m-0 p-0">
                <a class="delete_comment_btn" data-id="{comment_id}" href="<?php echo $_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING'] ?>&cid={comment_id}"><i class="bi bi-trash ms-1 text-danger"></i></a>
            </p>
        </div>
                    ...
                    if (response.meta_data) {
                        ...
                        if (data) {
                            Object.keys(data).forEach(function(key) {
                                if (key === "Is currently applying for the position of and If this is a consultant transition please specify") {
                                    if (data[key] && data[key] !== "-" && data[key] !== "NA") {
                                        apply_position = data[key];
                                    }
                                }
                                if (key === "Employee or consultant?") {
                                    if (data[key] && data[key] !== "-" && data[key] !== "NA") {
                                        e_or_c = data[key];
                                    }
                                }
                ...
                if (response != '') {
                    ...
                    loadFile(
                        ...
                        function(error, content) {
                            ...
                            link.download = `${cus_company}_${order_id}_${vasc_id ?? ''}_${name_ini.toUpperCase()}_${currentDate}.docx`;
                            link.click();
                        }
                    );
                }
