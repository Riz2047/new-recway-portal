...
// Live search companies (used by admin add-customer/company input autocomplete)
if (isset($_POST['search_company']) && $_POST['search_company'] !== '') {
    $term = trim($_POST['search_company']);
    try {
        global $conn;
        $query = "SELECT DISTINCT company FROM company_manager WHERE company LIKE :term ORDER BY company ASC LIMIT 20";
        $stmt = $conn->prepare($query);
        $stmt->execute([':term' => $term . '%']);
        $rows = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);
        // return array of objects with company property for compatibility
        $out = [];
        foreach ($rows as $r) {
            $out[] = ['company' => $r];
        }
...
if (isset($_POST['add_custom_message']) && ! empty($_POST['add_custom_message'])) {
    if (! empty($_POST['name']) && ! empty($_POST['message'])) {
        try {
            // Get the raw message data - don't strip or escape HTML
            $name = trim($_POST['name']);
            $message = $_POST['message']; // Keep as-is for HTML content

            // Use direct PDO to ensure proper handling of long text
            global $conn;
            $query = "INSERT INTO custom_email_template (name, message) VALUES (?, ?)";
            $stmt = $conn->prepare($query);
...
if (isset($_POST['update_custom_message']) && ! empty($_POST['update_custom_message'])) {
    if (! empty($_POST['u_id'])) {
        try {
            // Get the raw message data - don't strip or escape HTML
            $name = trim($_POST['name']);
            ...
            // Use direct PDO to ensure proper handling of long text
            global $conn;
            $query = "UPDATE custom_email_template SET name = ?, message = ? WHERE id = ?";
            $stmt = $conn->prepare($query);
            $stmt->bindParam(1, $name, PDO::PARAM_STR);
            $stmt->bindParam(2, $message, PDO::PARAM_STR);
            ...
            if ($result) {
                // Update all company_manager records that use this template
                $updatedCount = 0;
                if ($name === 'under_investigation') {
                    // Update ALL company_manager records' email_template column with the new HTML
                    $updateQuery = "UPDATE company_manager SET email_template = ?";
                    $updateStmt = $conn->prepare($updateQuery);
                    $updateStmt->execute([$message]);
                    $updatedCount = $updateStmt->rowCount();
                } elseif ($name === 'approved') {
                    // Update ALL company_manager records' email_template_approved column with the new HTML
                    $updateQuery = "UPDATE company_manager SET email_template_approved = ?";
                    $updateStmt = $conn->prepare($updateQuery);
                    $updateStmt->execute([$message]);
                    $updatedCount = $updateStmt->rowCount();
                }

                echo json_encode(['success' => true, 'updated_records' => $updatedCount]);
            } else {
    ...
    if (isset($_POST['can_id']) && ! empty($_POST['can_id'])) {
        if (isset($_FILES['file_1']['name']) && ! empty($_FILES['file_1']['name'][0])) {
            ...
            if (count($existingFiles) > 10) {
                $existingFiles = array_slice($existingFiles, -10);
            }
...
if (isset($_POST['get_par_department_stub']) && ! empty($_POST['get_par_department_stub'])) {
    $customers = findAllByQuery('SELECT * FROM customers WHERE id = ' . $_POST["id"]);
    $departments = findAllByQuery('SELECT * FROM departments WHERE dep_cus_id = ' . $_POST["id"] . ' AND dep_trash = 0');
    $services = findAllByQuery('SELECT * FROM customer_services WHERE cus_id = ' . $_POST["id"] . ' GROUP BY service_id');
    $permissions = findAllByQuery('SELECT * FROM user_allowed_permissions WHERE user_id = ' . $_POST["id"] . ' AND user_type = 2');
    $company_manager = findAllByQuery('SELECT * FROM company_manager WHERE cus_id = ' . $_POST["id"]);
    $standard_billing_details = findAllByQuery('SELECT * FROM standard_billing_details WHERE cus_id = ' . $_POST["id"]);
    echo json_encode(['customers' => $customers, 'departments' => $departments, 'services' => $services, 'permissions' => $permissions , 'company_manager' => $company_manager, 'standard_billing_details' => $standard_billing_details]);
}
...
if (isset($_POST['add_company_manager']) && ! empty($_POST['add_company_manager'])) {
    ...
    if (isset($_POST['cus_id']) && ! empty($_POST['cus_id'])) {
        ...
        $email_temp_approved = $_POST['email_temp_approved'];
        ...
        $insertQuery = "INSERT INTO company_manager (company, cus_id, email_template, email_template_approved, can_view_report) VALUES (:company, :cus_id, :email_template, :email_template_approved, :can_view_report)";
        $insertStmt = $conn->prepare($insertQuery);
        $insertStmt->execute([':company' => $company, ':cus_id' => $cus_id, ':email_template' => $email_temp, ':email_template_approved' => $email_temp_approved, ':can_view_report' => $enable_function]);
            ...
            if ($cus_id) {
                ...
                if ($company) {
                    ...
                    if (! empty($managerRecords)) {
                            ...
                            if ($managerRecord && isset($managerRecord['cus_id'])) {
                                ...
                                if ($customerRecord) {
                                    if (isset($managerRecord['can_view_report']) && ! empty($managerRecord['can_view_report'])) {
                                        // Fetch candidate first to check status
                                        $query = "SELECT * FROM candidates WHERE id = :can_id";
                                        $stmt = $conn->prepare($query);
                                        $stmt->execute([':can_id' => $can_id]);
                                        $candidate = $stmt->fetch();
                                        // Determine which email template to use based on candidate status

                                        $emailTemplate = '';
                                        if ($candidate->status == 4) {
                                            // Status 4 = Approved, use email_template_approved
                                            if (isset($managerRecord['email_template_approved']) && ! empty($managerRecord['email_template_approved'])) {
                                                $emailTemplate = $managerRecord['email_template_approved'];
                                            }
                                        } elseif ($candidate->status == 6) {
                                            // Status 6 = Under Investigation, use email_template
                                            if (isset($managerRecord['email_template']) && ! empty($managerRecord['email_template'])) {
                                                $emailTemplate = $managerRecord['email_template'];
                                            }
                                        }
                                        // Only proceed if we have a valid template
                                        if (! empty($emailTemplate) && strlen(trim($emailTemplate)) > 0) {
                                            ...
                                            $body = replace($emailTemplate, $customerRecord['name'], $candidate->name . " " . $candidate->surname, $customerRecord['company'], $service->title, ! empty($staff) ? $staff->name : '', '', '', $status->status, $currentDateTime, $candidate->order_id, '', '', '', $candidate->vasc_id, $service->title, ! empty($place) ? $place->name : '');
                                            // Create a DateTime object for Sweden's timezone
                                            $swedenTimezone = new DateTimeZone('Europe/Stockholm');
                                            $swedenTime = new DateTime('now', $swedenTimezone);
                                            $currentTime = $swedenTime->format('H:i:s');
                        ...
                        // First, get the company from the record with the given cus_id
                        $query = "SELECT `company` FROM company_manager WHERE cus_id = :cus_id LIMIT 1";
                        $stmt = $conn->prepare($query);
                        $stmt->execute([':cus_id' => $cus_id]);
                        $firstRecord = $stmt->fetch(PDO::FETCH_ASSOC);

                        if ($firstRecord && ! empty($firstRecord['company'])) {
                            $company = $firstRecord['company'];
                            $query = "SELECT * FROM company_manager WHERE company = :company AND can_view_report = 1";
                            $stmt = $conn->prepare($query);
                            $stmt->execute([':company' => $company]);
                            $managerRecords = $stmt->fetchAll(PDO::FETCH_ASSOC);
                        } else {
...
if (isset($_POST['delete_comment']) && ! empty($_POST['id'])) {
    ...
    if ($row && isset($row)) {
        ...
        // echo $newComment;
        $delete = $conn->prepare("DELETE FROM history WHERE id = :id");
...
if (isset($_POST['add_news_report']) && ! empty($_POST['add_news_report'])) {
    if (! empty($_POST['title'])) {
        ...
        if (isset($_FILES['pdf_file']['name']) && ! empty($_FILES['pdf_file']['name'])) {
            ...
            $fileExtension = pathinfo($originalFileName, PATHINFO_EXTENSION);
            $fileName = time() . '-' . uniqid() . '.' . $fileExtension;
            move_uploaded_file($_FILES['pdf_file']['tmp_name'], './../../uploads/' . $fileName);
        }
        ...
        $last_id = insert('news_reports', $data);
...
if (isset($_POST['get_news_report']) && ! empty($_POST['get_news_report'])) {
    if (isset($_POST['id'])) {
...
if (isset($_POST['update_news_report']) && ! empty($_POST['update_news_report'])) {
    if (! empty($_POST['title']) && ! empty($_POST['id'])) {
        ...
        if (isset($_FILES['pdf_file']['name']) && ! empty($_FILES['pdf_file']['name'])) {
            ...
            $fileExtension = pathinfo($originalFileName, PATHINFO_EXTENSION);
            $fileName = time() . '-' . uniqid() . '.' . $fileExtension;
            if (move_uploaded_file($_FILES['pdf_file']['tmp_name'], './../../uploads/' . $fileName)) {
                $data['pdf_file'] = $fileName;
            }
        }

        update('news_reports', $data, 'id', $id);
        echo json_encode(['success' => true]);
    } else {
        echo json_encode(['error' => 'Title and ID are required']);
    }
}

if (isset($_POST['delete_news_report']) && ! empty($_POST['delete_news_report']) && ! empty($_POST['id'])) {
    // Optional: Delete physical file
    $stmt = $conn->prepare("SELECT pdf_file FROM news_reports WHERE id = ?");
    $stmt->execute([$_POST['id']]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    delete('news_reports', 'id', $_POST['id']);
    echo json_encode(['success' => true]);
}
