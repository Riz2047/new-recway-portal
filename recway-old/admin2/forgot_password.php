...
if (isset($_POST['reset_password'])) {
    if (!empty($_POST['new_password'])  && !empty($_POST['confirm_password']) && $_POST['new_password'] == $_POST['confirm_password']) {
        $user = findByQuery("SELECT * FROM admin WHERE email = '{$_SESSION['forgot_p_email']}' LIMIT 1");
        update("admin", ["password" => password_hash($_POST['confirm_password'], PASSWORD_BCRYPT)], "email", $_SESSION['forgot_p_email']);
        $_SESSION['admin'] = $user;
