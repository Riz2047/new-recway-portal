...
if (isset($_POST['update_customer'])) {
    ...
    $query = 'UPDATE customers SET name = ?, email = ?, phone = ?, company = ?, cost_place = ?, statuses = ?, send_security_report = ?, groups = ?, reg_email = ?, combine_bk_and_security = ?, combine_status = ?,combine_interview_id = ?, invoice_period = ? WHERE id = ?';
    $stmt = $conn->prepare($query);
    $res = $stmt->execute([$name, $email, $phone, $company, $cost_place, $statusStr, $send_report, $groupid, $changed_registration_email, $combine_bk_and_security, $combine_status, $combine_interview_id, $invoice_period, $_GET['id']]);
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12 mt-3">
                                    <input class="form-check-input" id="enable_function" type="checkbox"
                                        name="enable_function" value="1" <?php if (! empty($selected_manger)) { ?>
                                        <?php if (! empty($selected_manger['can_view_report'])) { ?> checked <?php } ?> <?php } ?>>
