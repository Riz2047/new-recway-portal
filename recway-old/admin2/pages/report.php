...
if (isset($idArray[0]) && ! empty($idArray[0]) && $idArray[0] == 'service_id') {
    ...
    $report_html = findByQuery("SELECT * FROM customer_reports_html WHERE cus_id = '{$cus_id}' AND interview_id = {$service_id} AND `lang` = 'en'");
...
Niklas is most active on Linkedin, where he regularly participates in professional discussions and networking. Facebook and Instagram are mainly used to keep in touch with friends and acquaintances, while activity on Tiktok is limited. No suspicious activity or deviations have been detected on any of Niklas' social media accounts. After an extensive and professional background check, it can be determined that Niklas has a reliable and credible background without any deviations or negative indications. The personal data has been verified and confirmed as correct and convincing.
                                            ...
                                            <div class="comment text mb-3 grab-elements" style="">
                                                ...
                                                <div class="col-lg-12">
                                                    ...
                                                    <textarea name="comment_description[]" id="" required="" placeholder="Text description" rows="3" class="w-100 sign-textarea mb-3 comment_descriptions" spellcheck="true">The report may only be used by the Customer and may not be distributed to others. Recway is responsible not against anyone other than the Customer for the content of the report or for other use of the report than in connection with a background check. Recway is not responsible for any errors in them sources we obtain information from. källor vi hämtar uppgifter från. </textarea>
...
However, it is important to note that Recway cannot identify or discover all possible information about the candidate. The background checks provide a thorough assessment based on available information, but there is no guarantee that all relevant information can be obtained.</textarea>
                            ...
                            <?php if (isset($for_customer) && ! empty($for_customer)) { ?>
                                ...
                                <div class="col-lg-6 ">
                                    <button type="button" class="btn-primary-sm bg-primary w-100 mt-4 mx-0"
                                        onclick="save_report_for_customer('<?= $cus_id ?>')"><a>Save Report</a></button>
                                </div>
                            <?php } else { ?>
                            ...
                            <?php } else { ?>
                                ...
                                <div class="col-lg-6 ">
                                    ...
                                    <button type="button" id="save_comp" class="btn-primary-sm bg-primary w-100 mt-4 mx-0"><a>Save Report Template For Customer</a></button>
...
<script>
    ...
    $(".report-btn").click(function() {
        ...
        function addInformation() {
            ...
            // doc.addImage("../assets/" + statusImages[result], 'WEBP', doc.internal.pageSize.width - 50, y, 45, 45)
