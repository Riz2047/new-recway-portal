...
// Pehli query se data fetch karna
// $query = "SELECT `can_msg_2` FROM `messages` WHERE cus_id = ? AND interview_id = ?";
// $stmt = $conn->prepare($query);
// $stmt->execute([$_GET['id'], $candidate->interview_id]);
// $message = $stmt->fetch();
// Agar pehli query se data nahi mila, to id 1 wale record ka data le aao
// if (!$message) {
//     $query = "SELECT `can_msg_2` FROM `messages` WHERE cus_id = 1 AND interview_id = ?";
//     $stmt = $conn->prepare($query);
...
$query = "SELECT * FROM emails WHERE order_id = ? ORDER BY id DESC";
$stmt = $conn->prepare($query);
...
if (! empty($comments)) {
    foreach ($comments as $comment) {
        if ($comment->author_id != $adminId) {
            $readBy = explode(',', $comment->read_by_admin ?? '');
...
?>
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <p class="f-14 w-700 text-black up_company">
                                            <?php echo ! empty($customer->company) ? $customer->company : 'Null' ?>
                                        </p>
                                    </div>
                                ...
                                <div class="col-lg-6 order-lg-2 order-1">
                                    <div class="candidate-profile mx-auto mb-2">
                                            ...
                                            <?php foreach ($history as $h): ?>
                                                <li>
                                                    <div class="time" <?php if (strtotime($h->date_time) > time()) { ?>
                                                        ...
                                                        <span class="bi bi-trash ms-1 text-danger"
                                                            onclick="delete_comment(<?= $h->id ?>, this)"
                                                            style="float:right;cursor: pointer;"></span>
                                                    </div>
                                                    <p class="f-14 w-500"><?php echo $h->desc ?>
                                                    </p>
                                                    <i><small
                                                            class="m-0 p-0 comment-cell"><?php echo ! empty($h->comment) ? 'Comment: ' . $h->comment : '' ?></small></i>
                                                </li>
                                            <?php endforeach; ?>
                                        <?php endif; ?>
                                    </ul>
            ...
            <div class="col-lg-12 px-lg-0 mb-lg-0 mb-3 ">
                <div class="white-box">
                    <div class="tab2">
                        ...
                        <button class="tablinks2 f-14 w-700 " onclick="editCity(event, 'update-customer')">Update
                            Customer</button>
...
?>
                    ...
                    <div id="update-customer" class="tabcontent2 ">
                        <?php
$upd_customer = findAllByQuery('SELECT customers.* from customers LEFT JOIN customer_services ON customers.id = customer_services.cus_id WHERE customer_services.service_id = ' . $candidate->interview_id . ' GROUP BY customers.id');
?>
                        <div class="container">
                            <div class="row">
                                <form class="update-form" method="post">
                                    <div class="col-md-12 mb-3" id="">
                                        ...
                                        <select id="" name="u_cus" class="form-control mb-3 filter-select"
                                            onchange="get_cus_email_temp(this)"
                                            data-service="<?= $candidate->interview_id ?>">
                                            <?php if (! empty($upd_customer)): ?>
                                                <option value=""></option>
                                                <?php foreach ($upd_customer as $st): ?>
                                                    <option value="<?php echo $st->id ?>" data-comp="<?= $st->company ?>">
                                                        <?php echo $st->name ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            <?php endif; ?>
                                        ...
                                        <div class="col-lg-12 p-0 mb-3">
                                            <input type="hidden" name="can_id" value="<?= $candidate->order_id ?>">
                                            <label class="form-label">Email Template</label>
                                            <textarea class="sign-textarea w-100" name="u_cus_email"
                                                rows="6"></textarea>
                                        </div>
                                    </div>
                                    <div id="update_cus_msg" class="text-center"></div>
                                    <div class="d-flex justify-content-end">
                                        <button type="submit" name="update" id="update_cus_btn"
                                            class="btn-primary bg-primary">Update</button>
                                    </div>
                                </form>
                            </div>
                                    ...
                                    <div class="table-section w-100 p-0">
                                        <div class="table-div w-100">
                                            ...
                                            <div class="col-md-12 d-flex justify-content-end">
                                                <select name="" id="" class="form-control" style="width: 200px; margin-right: 10px;">
                                                    <option value="new_order">New Order</option>
                                                </select>
                                                <button class="btn btn-info btn-sm" id="duplicateBtn">Duplicate</button>
                                            </div>
                                        </div> -->
                                            <form action="" method="post" id="d-form">
                                                <table id="email_table" class="display Table w-100">
                                                    <thead>
                                                        <tr class="email_row">
                                                    ...
                                                    <tbody>
                                                        <?php if (! empty($emails)): ?>
                                                            ...
                                                            <?php foreach ($emails as $email): ?>
                                                                <?php if ($email->user_type == "Candidate" && $email->order_id == $candidate->order_id): ?>
                                                                    <?php
                        ...
                        <div class="container">
                            <form class="update-form" method="post" enctype="multipart/form-data">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md-12 mb-3 " id="place">
                                    ...
                                    <div class="col-md-12 mb-3">
                                        ...
                                        <select id="interview" name="service" class="form-control filter-select"
                                            onchange="check_p_c(this); check_combine_bk_and_security()">
...
?>
                                        ...
                                        <?php } ?>
                                        <!-- <div class="col-lg-12 mb-3 service_cost" style="display: none;">
                                ...
                                <div class="d-flex justify-content-end">
                                    <button type="submit" name="update_status"
                                        class="btn-primary bg-primary  report-btn2 report-btn-update">Update</button>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="container">
                            <div class="row">
                                <div class="record d-flex col-8 justify-content-between">
                                    <input class="form-check-input invoice_sent" name="invoice_sent"
                                        id="invoice_checkbox<?php echo $candidate->id ?>" type="checkbox" <?php echo $candidate->invoice_sent == 1 ? 'checked' : '' ?>
                                        data-id="<?php echo $candidate->id ?>" onchange="fun_invoice_date(this)">
                                    <label class="form-check-label f-14 w-600 text-grey mb-0 pb-0 "
                                        for="invoice_checkbox<?php echo $candidate->id ?>">
                                        Invoice Sent
                                    </label>
                                ...
                                <div class="col-md-8">
                                    <input class="form-check-input" id="reported_to_sm<?php echo $candidate->id ?>"
                                        ...
                                        data-rid="<?php echo $candidate->id ?>"
                                        onclick="check_reported_by(this),change_in_table(this)">
                                    <label class="form-check-label f-14 w-600 text-grey mb-0 pb-0 "
    ...
    <div id="report-section">
        ...
        <div class="row">
            <div class="col-md-12" id="city_text_msg" style="display:none">
                <p class="text-danger">Please Fill the Interview Place Filed !!</p>
            </div>
...
<script>
    ...
    var typeStatus = $('#change-status').val();
...
<script>
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    for (const h of response.history) {
                        ...
                        var deleteIcon = `<span class="bi bi-trash ms-1 text-danger"
                     onclick="delete_comment(${h.id}, this)"
                     style="float:right;cursor: pointer;"></span>`;
                        var dateHtml = formatDate(h.date_time) + ' ' + deleteIcon;
                        timeline = timeline.replace("{date}", dateHtml)
                            .replace("{description}", h.desc)
                        if (h.comment !== null && h.comment !== "") {
                            timeline = timeline.replace("{comment_li}", '<i><small class="m-0 p-0 comment-cell">{comment}</small></i>')
    ...
    //         $.ajax({
    //             url: '../includes/pages.php', // Yahan aap apni PHP file ka URL den
    //             type: 'POST',
    //             data: { type: 'send_email', id }, // Yahan data pass karen agar zaroorat ho
    //             success: function(response) {
    //                 // Response handle karen
    //                 // alert("Duplicate action successful: " + response);
    //             },
    //             error: function(xhr, status, error) {
    //                 // Error handle karen
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (response.candidate.cv) {
                        ...
                        var rep_html = `<p class="mb-0 w-100 f-18 px-2 py-3">No Document</p>`
                        $("#pdfModal").find(".modal-body").html(rep_html)
                    }
                    $("#reportModal").find(".modal-body").empty()
                    if (response.candidate.report) {
                        var rep_html = `<p style="overflow: hidden;white-space: nowrap; text-overflow: ellipsis" class="mb-0 w-100 f-18 p-0 pt-1"><a target="_blank" href="../report-uploads/` + response.candidate.report + `" style="cursor: pointer" class="text-success">` + response.candidate.report + `</a></p>`
                        $("#reportModal").find(".modal-body").html(rep_html)
                    } else {
                        var rep_html = `<p class="mb-0 w-100 f-18 px-2 py-3 no-report">No Report Submitted</p>`
                        $("#reportModal").find(".modal-body").html(rep_html)
                    }
                } else {
                    alert("Error fetching data")
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('a[data-id="' + id + '"]').closest('tr').find('.staff_show').text(staff.name);
                    fetchHistory()
                } else {
    ...
    function updateStatus(that) {
        ...
        // if ($('.service_cost').is(':visible')) {
        //     if ($('#travelling_cost').val() == '') {
        ...
        // console.log(interview_id,'helllo interview_id');
        // console.log(interview_id.toString(),'helllo interview_id.toString()');
                    ...
                    if (response.status && response.status.id && combine_status_array.includes(response.status.id.toString())) {
                        ...
                        if (response.combine_interview_place == 1) {
                            if (response.customer &&
                                ...
                                console.log('Status 22 detected - updating to combine interview flow');
                                var combine_id = response.candidate.combine_interview_id == '0' || response.candidate.combine_interview_id == 0 ? response.customer.combine_interview_id : response.candidate.combine_interview_id
    ...
    $(document).ready(function() {
        ...
        $("#uploadcv").on('submit', function(e) {
            ...
            `;
                ...
                error: function() {
                    Swal.fire("Error", "An error occurred during upload. Please try again.", "error");
                }
            });
        });
        $("#uploadinterview_template").on('submit', function(e) {
            ...
            Swal.fire({
                title: 'Uploading...',
                text: 'Please wait while the template is being uploaded.',
                allowOutsideClick: false,
                showConfirmButton: false,
                willOpen: () => {
                    Swal.showLoading();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response != '') {
                        ...
                        Swal.fire("Uploaded Successfully", "Interview template has been uploaded.", "success");
                    }
                },
                error: function() {
                    Swal.fire("Error", "An error occurred during upload. Please try again.", "error");
    ...
    function change_in_table(obj) {
        ...
        if ($(obj).is(':checked')) {
            $('#reported-' + checkbox_id).prop('checked', true)
        } else {
    ...
    function get_cus_email_temp(obj) {
        var cus = $(obj).val();
        var ser_id = $(obj).data('service');
        $.ajax({
            type: "POST",
            url: "./includes/table_ajax.php",
            data: {
            ...
            success: function(response) {
                if (response != '') {
                    response = JSON.parse(response)
                    if (response[0].cus_msg) {
                        $('textarea[name="u_cus_email"]').val(response[0].cus_msg)
                    } else {
                        $('textarea[name="u_cus_email"]').val('')
                    }
                } else {
    ...
    $("#update_cus_btn").on("click", function(e) {
        e.preventDefault()
        $(this).prop("disabled", true);
        $("#update_cus_msg").html($("#spinner").html())
        var formData = new FormData($(this).closest("form")[0]);
        formData.append('type', 'update_cus');
        var that = $(this)
        $.ajax({
            type: "POST",
            url: "./includes/table_ajax.php",
            ...
            success: function(response) {
                if (response) {
                    ...
                    that.prop("disabled", false);
                    $("#update_cus_msg").html("")
                    $('a[data-id="' + id + '"]').closest('tr').find('.customer_show').text($.trim($('select[name="u_cus"]').find('option:selected').text()));
                    $('.up_company').text($.trim($('select[name="u_cus"]').find('option:selected').data('comp')));
                    $('a[data-id="' + id + '"]').closest('tr').find('.company_show').find('a').text($.trim($('select[name="u_cus"]').find('option:selected').data('comp')));
                    $("#updated_customer").val(response.combine_bk_and_security);
                    $('#updated_combine_statuses').val(response.combine_status);
                    check_combine_bk_and_security();
                }
            },
            error: function(e) {
    ...
    function delete_comment(id, btn) {
        if (confirm("Are you sure you want to delete this comment?")) {
            $.ajax({
                ...
                success: function(response) {
                    if (response) {
                        flash("successMsg", "History Deleted!");
                ...
                error: function() {
                    flash("errorMsg", "Error updating comment!");
                }
            });
        }
    }
...
<script>
    ...
    async function fillStaffNameInPDF(obj) {
        ...
        const existingPdfBytes = await fetch("./../assets/docx/Eliviio.pdf")
        ...
        $.ajax({
            ...
            success: async function (response) {
                ...
                // SAFE BACKEND VALUES
                // =========================
                var order_id = safe(response.order_id);
                var firstName = safe(response.name);
                var surname = safe(response.surname);
                var vasc_id = safe(response.vasc_id);
                var phone_number = safe(response.staff_number);
                var refperson = safe($('#hiring_manager_name').val());
                var cus_name = safe(response.cus_name).replace(/\s+/g, ' ').trim();
                var cus_company = safe(response.cus_company).trim();
                var place = safe(response.place_name);
                var staff = safe(response.staff);
                var interview_date = safe(response.booked);
                var cri_check = response.criminal_record;
                var ssn = safe(response.security);
                var soc_check = response.social;
                var eco_check = response.economy;
                var bk_date = safe(response.background_check_date);
                var staff_email = safe(response.staff_email);
                var can_phone = safe(response.phone);

                // =========================
                // Name Handling (Safe)
                // =========================
                var name_ini =
                    firstName.replace(/\s+/g, '').substring(0, 1) +
                    surname.replace(/\s+/g, '').substring(0, 1);

                var can_name = (firstName + " " + surname).trim();
                var name = (firstName.toUpperCase()).trim();

                // =========================
                // META DATA SAFE PARSE
                // =========================
                var apply_position = "N/A";
                var department = "N/A";
                ...
                if (response.meta_data) {
                    try {
                        const data = JSON.parse(response.meta_data);

                        if (data["Avdelning/Enhet"] && data["Avdelning/Enhet"] !== "-" && data["Avdelning/Enhet"] !== "NA") {
                            apply_position = safe(data["Avdelning/Enhet"]);
                        }

                        if (data["Befattning/arbete i företaget"] && data["Befattning/arbete i företaget"] !== "-" && data["Befattning/arbete i företaget"] !== "NA") {
                            department = safe(data["Befattning/arbete i företaget"]);
                        }

                    } catch (e) {
                        console.warn("Meta data parse error:", e);
                    }
                }
                ...
                const setField = (fieldName, value) => {
                    try {
                        form.getTextField(fieldName).setText(safe(value));
                    } catch (e) {
                ...
                // SET FORM FIELDS
                // =========================
                setField("Föroch efternamn", staff);
                setField("Titel och bolag", 'Recway AB');
                setField("Telefonnummer", phone_number);
                setField("Epost", staff_email);
                setField("SvarDatum för intervju", interview_date);
                setField("SvarPlats för intervju", place);
                setField("SvarSamtliga förnamn tilltalsnamn med versaler", name);
                setField("SvarEfternamn", surname.toUpperCase());
                setField("SvarPersonnummersamordningsnummer 12 siffror", ssn);
                setField("SvarTelefonnummer privat", can_phone);
...
<script>
    ...
    $("#file_1").on("change", function() {
        ...
        if (existingCount + newFiles.length > 10) {
            Swal.fire("Limit Exceeded", "You can only have a maximum of 10 files total. You already have " + existingCount + ".", "warning");
