...
if (isset($_POST['update_customer'])) {
    ...
    $query = 'UPDATE customers SET name = ?, email = ?, phone = ?, company = ?, cost_place = ?, statuses = ?, send_security_report = ?, remainder_email_template, send_email = ?, combine_bk_and_security = ?, combine_status = ?,combine_interview_id = ?, invoice_period = ? WHERE id = ?';
    $stmt = $conn->prepare($query);
    $res = $stmt->execute([$name, $email, $phone, $company, $cost_place, $statusStr, $send_report, $remainder_email_template, $send_email, $combine_bk_and_security, $combine_status, $combine_interview_id, $invoice_period, $_GET['id']]);
...
$cus_id = $_GET['id'];
$query = "SELECT i.id ,i.title FROM `interviews` i JOIN `customer_services` cs ON cs.service_id = i.id WHERE cs.cus_id = '$cus_id'";
$stmt = $conn->prepare($query);
$stmt->execute();
$customer_specific_services = $stmt->fetchAll();
...
<style>
    .select2-container--open {
        ...
        /* Bootstrap modal z-index is 1040 */
    }
            ...
            <div class="col-lg-12 px-lg-0 mb-lg-0 mb-3 ">
                <div class="white-box-p-0 h-100">
                    <div class="tab">
                        ...
                        <button class="tablinks f-14 w-700 " id="defaultOpen" onclick="openCity(event, 'edit')">Edit
                            Customer</button>
                        <button class="tablinks f-14 w-700 " id="statusManager"
                            onclick="openCity(event, 'status_manager')">Status Manager</button>
                        <button class="tablinks f-14 w-700 " id="billing_details"
                            onclick="openCity(event, 'sbilling_details')">Standard Billing Details</button>
                        <button class="tablinks f-14 w-700 " id="departmentsOpen"
                            onclick="openCity(event, 'departments')">Departments</button>
                        <button class="tablinks f-14 w-700 " id="departmentUsersOpen"
                            onclick="openCity(event, 'department_users')">Department Users</button>
                        ...
                        <button class="tablinks f-14 w-700 totalInvoicedBtn"
                            onclick="openCity(event, 'invoiced')">Invoiced</button>
                        <button class="tablinks f-14 w-700" onclick="openCity(event, 'background-qs')">Background
                            Questions</button>
                        <button class="tablinks f-12 w-700" onclick="openCity(event, 'form_builder')">Form
                            Builder</button>
                        <button class="tablinks f-12 w-700" onclick="openCity(event, 'bk_reports')">Reports</button>
                        <button class="tablinks f-12 w-700" id="addCuOpen"
                            onclick="openCity(event, 'additional_customers')">Additional Customers</button>
                        <button class="tablinks f-12 w-700" id="remainder_email_tab"
                            onclick="openCity(event, 'remainder_email_box')">Remainder Emails</button>
                        <button class="tablinks f-12 w-700" id="addCuOpen"
                            onclick="openCity(event, 'service_cost')">Service Cost</button>
                    </div>
...
?>
                                        <h1 class="f-26 w-700 text-white m-0 p-0 font-secondary">
                                            <?php echo $first_name_initial . $last_name_initial ?>
                                        </h1>
                                    </div>
                                    <div class="candidate-info ">
                                        ...
                                        <div class="status-active px-3 py-1 f-18 my-2 mx-auto"
                                            style="background-color: #4C6EF8">Total Orders
                                            <?php echo ! empty($orders) ? $orders->total_orders : 0 ?>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class="total-card shadow2">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div class="d-flex flex-column align-items-start">
                                                    ...
                                                    <h1 class="text-numer mb-0 pb-0">
                                                        <?php echo ! empty($orders) ? $orders->total_invoiced : 0 ?>
                                                    </h1>
                                                </div>
                                                <div class="icon-card">
                                        ...
                                        <div class="total-card shadow2">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div class="d-flex flex-column align-items-start">
                                                    ...
                                                    <h1 class="text-numer mb-0 pb-0">
                                                        <?php echo ! empty($orders) ? $orders->total_approved : 0 ?>
                                                    </h1>
                                                </div>
                                                <div class="icon-card">
                                        ...
                                        <div class="total-card shadow2">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div class="d-flex flex-column align-items-start">
                                                    ...
                                                    <h1 class="text-numer mb-0 pb-0">
                                                        <?php echo ! empty($orders) ? $orders->total_canceled : 0 ?>
                                                    </h1>
                                                </div>
                                                <div class="icon-card">
                                            ...
                                            <div class="row mb-3">
                                                <div class="col-lg-6 mb-3">
                                                    ...
                                                    <input type="text" class="form-control" name="name"
                                                        value="<?php echo $customer->name ?>" required id="name">
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label class="form-label" for="email">Email</label>
                                                    <input type="email" class="form-control" name="email"
                                                        value="<?php echo $customer->email ?>" required id="email">
                                                    <input type="hidden" required name="old_email"
                                                        value="<?php echo $customer->email ?>"
                                                        class="sign-input w-100 mb-3" placeholder="Email Address ">
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label class="form-label" for="phone">Phone</label>
                                                    <input type="text" class="form-control" name="phone"
                                                        value="<?php echo $customer->phone ?>" required id="phone">
                                                </div>
                                                <!-- <div class="col-lg-6 mb-3">
                                                    <label class="form-label" for="company">Company</label>
                                                    <input type="text" class="form-control" name="company"
                                                        value="<?php echo $customer->company ?>" required id="company">
                                                </div> -->
                                                <div class="col-lg-6 mb-3" style="position: relative;">
                                                    <label class="form-label" for="company">Company</label>
                                                    <input type="text" class="form-control" name="company" value="<?php echo $customer->company ?>" required id="company" autocomplete="off">
                                                ...
                                                <div class="col-lg-6">
                                                    <label class="form-label" for="cost_place">Organization
                                                        Number</label>
                                                    <input type="text" class="form-control" name="org_no"
                                                        value="<?php echo $customer->org_no ?>" required
                                                        id="cost_place">
                                                </div>
                                                <div class="col-lg-6">
                                                    <label class="form-label">Send result of the basic
                                                        investigation</label>
                                                ...
                                                <div class="col-lg-6 mb-3">
                                                    ...
                                                    <select name="parent_customer" id="parent_customer"
                                                        onchange="get_dep(this)" class="form-control filter-select">
                                                        <option value="">-Select Customer-</option>
                                                        <?php if (! empty($parent_customer)) { ?>
                                                            <?php foreach ($parent_customer as $par_customer) { ?>
                                                                <option value="<?= $par_customer->id ?>" <?php if ($customer->parent_id == $par_customer->id) { ?> selected
                                                                    <?php } ?>><?= $par_customer->name ?></option>
                                                            <?php } ?>
                                                        <?php } ?>
                                                    </select>
                                                </div>
...
?>
                                                <div class="col-lg-6 mb-3">
                                                    ...
                                                    <select name="cus_department " id="cus_department"
                                                        class="form-control filter-select">
                                                        <option value="">-Select Department-</option>
                                                        <?php if (! empty($parent_departent)) { ?>
                                                            <?php foreach ($parent_departent as $par_departent) { ?>
                                                                <option value="<?= $par_departent->dep_id ?>" <?php if ($customer->dep_id == $par_departent->dep_id) { ?> selected
                                                                    <?php } ?>><?= $par_departent->dep_name ?></option>
                                                            <?php } ?>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                                <div class="col-lg-12 mb-3">
                                                    ...
                                                    <select name="select_group[]" class="filter-select select2tag"
                                                        multiple>
                                                        <?php if (! empty($groups)) {
                                                            $exp_arr = null;
                                                            ...
                                                            <?php foreach ($groups as $group) { ?>
                                                                <option value="<?= $group->id ?>" <?php if (! empty($exp_arr)) { ?> <?php foreach ($exp_arr as $exp_arrs) { ?> <?php if ($exp_arrs == $group->id) { ?> selected <?php } ?> <?php } ?>
                                                                    <?php } ?>><?= $group->name ?></option>
                                                            <?php } ?>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                                <div class="col-md-4 mb-3">
                                                    <input class="form-check-input" id="send_email" type="checkbox"
                                                        name="send_email" value="1" <?= $customer->sent_email == 1 ? 'checked' : '' ?>>
                                                ...
                                                <div class="col-md-4 mb-3">
                                                    <input class="form-check-input" id="ellevio_report" type="checkbox"
                                                        name="ellevio_report" value="1" <?= $customer->ellevio_report == 1 ? 'checked' : '' ?>>
                                                ...
                                                <div class="col-md-4 mb-3">
                                                    <input class="form-check-input" id="timra_report" type="checkbox"
                                                        name="timra_report" value="1" <?= $customer->timra_report == 1 ? 'checked' : '' ?>>
                                                ...
                                                <div class="col-md-4 mb-3">
                                                    <input class="form-check-input" id="send_email_question" type="checkbox"
                                                        name="send_email_question" value="1" <?= $customer->send_email_question == 1 ? 'checked' : '' ?>>
...
?>
                                                        <?php if (! empty($combine_services)) { ?>
                                                            <?php foreach ($combine_services as $service) { ?>
                                                                <option value="<?= $service->id ?>"
                                                                    <?php if ($customer->combine_interview_id == $service->id) { ?> selected <?php } ?>><?= $service->title ?></option>
                                                            <?php } ?>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                                <div class="col-lg-12 mb-3">
                                                    ...
                                                    <textarea rows="5" class="sign-textarea w-100"
                                                        name="changed_registration_email"><?php if (! empty($customer->reg_email)) {
                                                        ...
                                                        <?php foreach ($permissions as $permission): ?>
                                                            <div>
                                                                <input class="form-check-input"
                                                                    id="<?php echo $permission->title ?>" <?php if (! empty($allow_permissions)) { ?> <?php foreach ($allow_permissions as $allow) { ?> <?php if ($allow->per_id == $permission->id) { ?> checked <?php } ?>
                                                                    <?php } ?> <?php } ?> type="checkbox" name="permissions[]"
                                                ...
                                                <?php if (! empty($servicesCats)): ?>
                                                    <?php foreach ($servicesCats as $servicesCat): ?>
                                                        ...
                                                        <div class="col-lg-3" id="required-status">
                                                            <label class="form-label">Status Required -
                                                                <?php echo $servicesCat->name ?></label>
                                                            <?php if (! empty($statuses3)): ?>
                                                                <?php foreach ($statuses3 as $status): ?>
                                                                    <div>
                                                                        <input <?php echo in_array($status->sID, $cusStatuses) ? 'checked' : '' ?> class="form-check-input" type="checkbox"
                                                                            id="<?php echo str_replace(' ', '-', $status->variable) ?><?php echo $status->sID ?>"
                                                                            name="statuses[]" value="<?php echo $status->sID ?>">
                                                                        <label class="form-label form-check-label"
                                                        ...
                                                        <?php foreach ($services as $service): ?>
                                                            <div>
                                                                <input class="form-check-input service_checkbox"
                                                                    id="<?php echo $service->id . $service->title ?>" <?php echo in_array($service->id, $allowed_services) ? 'checked' : '' ?> type="checkbox" name="services[]"
                                                                    value="<?php echo $service->id ?>">
                                                                <label class="form-label form-check-label"
                                                                    for="<?php echo $service->id . $service->title ?>"><?php echo $service->title ?></label>
                                                            </div>
                                                        <?php endforeach; ?>
                                                    <?php endif; ?>
                                                </div>
                                            ...
                                            <div class="d-flex justify-content-end">
                                                <button id="update_customer_btn" type="submit" name="update_customer"
                                            ...
                                            <div class="row mb-3">
                                                <div class="col-lg-12 mb-3">
                                                    <input class="form-check-input" id="remainder_email" type="checkbox"
                                                        name="remainder_email" value="1" <?= $customer->remainder_email == 1 ? 'checked' : '' ?>>
                                                    ...
                                                    <textarea rows="5" class="sign-textarea w-100"
                                                        name="remainder_email_template"><?php if (! empty($customer->remainder_email_template)) {
                                                ...
                                                <div class="col-lg-12 mb-3">
                                                    <input class="form-check-input" id="bk_remainder_email" type="checkbox"
                                                        name="bk_remainder_email" value="1" <?= $customer->bk_remainder_email == 1 ? 'checked' : '' ?>>
                                                    ...
                                                    <label class="form-label">Background Check Remainder Email
                                                        Template:</label>
                                                    <textarea rows="5" class="sign-textarea w-100"
                                                        name="bk_remainder_email_template"><?php if (! empty($customer->bk_remainder_email_template)) {
                        ...
                        <div class="container">
                            <div class="row">
                                ...
                                <div class="col-md-12 mt-3">
                                    ...
                                    <label for="email_of_report" class="d-block">Interview Report Upload Email Template (Note: Leave it empty if you don't want to send an email for interview report upload)</label>
                                    <?php
                                    $under_investigation_msg = '';
$approved_msg = '';
if (! empty($custom_email_template) && is_array($custom_email_template)) {
    foreach ($custom_email_template as $tpl) {
        if ($tpl['name'] === 'under_investigation') {
            $under_investigation_msg = $tpl['message'];
        }
        if ($tpl['name'] === 'approved') {
            $approved_msg = $tpl['message'];
        }
    }
}
?>
                                    <div class="row">
                                        <div class="col-md-6">
                                            <label for="under_investigation">Under Investigation Email Template</label>
                                            <textarea name="email_of_report" class="form-control" id="email_of_report"><?php
                                                                                    if (! empty($selected_manger) && ! empty($selected_manger['email_template'])) {
                                                                                        echo htmlspecialchars($selected_manger['email_template']);
                                                                                    } else {
                                                                                        echo htmlspecialchars($under_investigation_msg);
                                                                                    }
?></textarea>
                                        </div>
                                        <div class="col-md-6">
                                            <label for="Approved">Approved Email Template</label>
                                            <textarea name="email_template_approved" class="form-control" id="email_template_approved"><?php
                if (! empty($selected_manger) && ! empty($selected_manger['email_template_approved'])) {
                    echo htmlspecialchars($selected_manger['email_template_approved']);
                } else {
                    echo htmlspecialchars($approved_msg);
                }
?></textarea>
                                        </div>
                                    </div>
                        ...
                        <div class="container">
                            <div class="row">
                                <div class="col-lg-12">
                                    ...
                                    <input type="text" name="standard_pref" class="form-control" id="billing_pref"
                                        value="<?= $standard_pref ?>">
                                </div>
                                <div class="col-lg-12">
                                    <label for="">Reference</label>
                                    <input type="text" name="standard_ref" class="form-control" id="billing_ref"
                                        value="<?= $standard_ref ?>">
                                </div>
                                <div class="col-lg-12">
                                    <label for="">Invoice Comment</label>
                                    <input type="text" name="standard_comment" class="form-control" id="billing_comment"
                            ...
                            <div class="row">
                                ...
                                <?php if ($cus_bk_services) { ?>
                                    <div class="col-md-6 pl-0">
                                        <a class="open-report btn bg-primary w-100 btn-sm ml-0" data-bs-toggle="modal"
                                            data-id="service_id,cus_id,<?= $cus_id ?>" data-modal="new"
                                            data-lang="sv">Report SV</a>
                                    </div>
                                    <div class="col-md-6 pr-0">
                                        <button type="button" class="open-report btn bg-primary w-100 btn-sm pr-1"
                                            data-bs-toggle="modal" data-id="service_id,cus_id,<?= $cus_id ?>"
                                            data-modal="new" data-lang="en">Report EN</button>
                                    </div>
                                <?php } ?>
                            </div>
                        </div>
                                ...
                                <div class="col-lg-12">
                                    <div class="table-section">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            ...
                                            <button class="btn btn-outline-blue btn-rounded btn-sm"
                                                onclick="openCity(event, 'add_department')">Add</button>
                                        </div>
                                        <table class="table table-bordered">
                                            <thead>
                                                <tr>
                                                ...
                                                <?php if (! empty($department)) { ?>
                                                    <?php foreach ($department as $dep) { ?>
                                                        <tr>
                                                            ...
                                                            <td style="width:6% !important">
                                                                <div class="dropdown">
                                                                    <button class="table-menu-btn mx-auto dropdownBtn"
                                                                        onclick="dropdown_open(this)" type="button"
                                                                        aria-expanded="false">
                                                                        <i class="bi bi-gear"></i>
                                                                    </button>
                                                                    <ul class="dropdown-menu p-2 ps-3 table-menu-btn-list">
                                                                        <li class="mb-1"><a href="#"
                                                                                onclick="openCity(event, 'update_department'),get_data(this)"
                                                                                data-id="<?= $dep->dep_id ?>" data-type="1"
                                ...
                                <div class="col-lg-12">
                                    <div class="table-section">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            ...
                                            <button class="btn btn-outline-blue btn-rounded btn-sm back_btn"
                                                onclick="openCity($('#departmentsOpen'), 'departments' )">Back</button>
                                        </div>
                                        <div class="d-flex align-items-center form-row mb-3">
                                            <label for="" class="label-lg">Department Name</label>
                                            <input type="text" name="name" placeholder="Department Name" required
                                                class="w-100 from-input">
                                            <input type="hidden" name="dep_cus_id" value="<?= $_GET['id'] ?>">
                                        </div>
                                        <div class="col-md-12 p-0 mb-3" style="border-right:0px !important">
                                            <label for="depar_select2" class="label-lg"
                                                style="border-right:0px !important">Department (Show orders of these
                                                departments to this also)</label>
                                            <div class="col-md-12 p-0">
                                                <select name="department" class="form-select select2" multiple
                                                    id="depar_select2" style="height: 33px;">
                                                    <?php if (! empty($department)) { ?>
                                                        <?php foreach ($department as $k => $depart) { ?>
                                                            <option value="<?= $department[$k]->dep_id ?>">
                                                                <?= $department[$k]->dep_name ?>
                                                            </option>
                                                        <?php } ?>
                                                    <?php } ?>
                                                </select>
                                        ...
                                        <div class="d-flex justify-content-end">
                                            <button type="button" class="btn btn-outline-blue btn-rounded btn-sm"
                                                id="add_department_btn" onclick="departments_data(this)">Add</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="table-section">
                                        ...
                                        <div class="d-flex align-items-center form-row mb-3">
                                            ...
                                            <input id="name" type="text" required name="name"
                                                placeholder="Department Name" class="w-100 from-input">
                                            <input type="hidden" name="up_dep_id">
                                        </div>
                                        <div class="col-md-12 p-0 mb-3" style="border-right:0px !important">
                                            <label class="label-lg" style="border-right:0px !important">Department (Show
                                                orders of these departments to this also)</label>
                                            <div class="col-md-12 p-0">
                                                <select name="child_department" class="form-select select2" multiple
                                                    style="height: 33px;">
                                                    <?php if (! empty($department)) { ?>
                                                        <?php foreach ($department as $k => $depart) { ?>
                                                            <option value="<?= $department[$k]->dep_id ?>">
                                        ...
                                        <div class="d-flex justify-content-end">
                                            <button type="button" class="btn btn-outline-blue btn-rounded btn-sm"
                                                id="update_department_btn"
                                                onclick="departments_data(this)">Update</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-lg-12">
                                    <div class="table-section">
                                        ...
                                        <div class="col-md-4 p-0 mb-4">
                                            ...
                                            <select id="" class="form-control filter-select"
                                                onchange="show_dep_users(this)">
                                    ...
                                    <div class="table-section">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-12 mt-1">
                                                <a href="customer-candidates.php?id=<?php echo $customer->id ?>"
                                                    style="text-decoration: none">
                                                    <div class="total-card shadow-sm">
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            <div class="d-flex flex-column align-items-start">
                                                                <h1 class="f-16 w-500">Total Orders</h1>
                                                                <h1 id="total_orders_count" class="f-22 w-800">
                                                                    <?php echo ! empty($candidates) ? count($candidates) : 0 ?>
                                                                </h1>
                                                            </div>
                                                            <div class="">
                                                        ...
                                                        <?php if (! empty($statuses)): ?>
                                                            <?php foreach ($statuses as $status): ?>
                                                                <tr id="<?php echo str_replace(' ', '-', $status->status) ?>">
                                                                    <th><a class="no-decoration text-black"
                                                                            href="history.php?id=<?php echo $_GET['id'] ?>&status=<?php echo $status->id ?>"><?php echo $status->status ?></a>
                                                                    </th>
                                ...
                                <div class="col-lg-12">
                                    <div class="table-section">
                                        ...
                                        <form action="" method="post" id="d-form">
                                            ...
                                            <table id="emails_table" data-table="customer_emails" class="display Table w-100">
                                                <thead>
                                                    <tr>
                                                    ...
                                                    <?php foreach ($messages as $col => $message): ?>
                                                        <div class="col-lg-6 mb-3">
                                                            ...
                                                            <div class="position-relative">
                                                                <textarea rows="5" class="sign-textarea w-100"
                                                                    name="<?php echo $col ?>"><?php echo $message ?></textarea>
                                ...
                                <div class="row questions-row">
                                    <div class="col-md-12">
                                        <button type="button" class="btn btn-primary float-right"
                                            onclick="add_question_type_base()">Add Question</button>
                                    </div>
                                    <div class="row" id="add-question" style="display:none">
                                        <div class="col-md-12">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <input type="radio" class="question_type" name="qs_type" checked
                                                        value="radio_opt">
                                                    <label>Radio Options</label>
                                                </div>
                                                <div class="col-md-6">
                                                    <input type="radio" class="question_type" name="qs_type"
                                                        value="free_text">
                                        ...
                                        <div class="col-md-12 text-right">
                                            <button type="button" class="btn btn-secondary btn-sm"
                                                onclick="add_question_type_base()">Close</button>
                                            <button type="button" class="btn btn-success btn-sm"
                                                onclick="add_question();">Add</button>
                                        </div>
                                    </div>
                                    <?php
                                    $qsno = 0;
?>
                                        ...
                                        <?php foreach ($customer_meta_data as $i => $meta_data) { ?>
                                            <?php if (isset($meta_data['type']) && ! empty($meta_data['type']) && $meta_data['type'] == 'radio') { ?>
                                                <div class="col-lg-12 mb-4 question_count">
                                                    <label for="">Question <?= ++$qsno ?>:</label> <button type="button"
                                                        class="btn btn-danger ml-5 mb-2" onclick="remove_question(this)"><span
                                                            class="bi bi-trash"></span></button>
                                                    <input type="text" name="qs[<?= $i ?>][qs]" <?php if (isset($meta_data['qs']) && ! empty($meta_data['qs'])) { ?>value="<?= $meta_data['qs'] ?>" <?php } ?>
                                                        class="form-control">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-8">
                                                            <button type="button" class="btn btn-primary" onclick="add_option(this)"
                                                                data-id="<?= $i ?>"><i class="bi bi-plus-lg"></i></button>
                                                        </div>
                                            ...
                                            <?php if (isset($meta_data['type']) && ! empty($meta_data['type']) && $meta_data['type'] == 'free_text') { ?>
                                                <div class="col-lg-12 question_count">
                                                    <label for="">Question <?= ++$qsno ?>:</label><button type="button"
                                                        class="btn btn-danger ml-5 mb-2" onclick="remove_question(this)"><span
                                                            class="bi bi-trash"></span></button>
                                                    <input type="text" name="qs[<?= $i ?>][qs]"
                                                        value="What does security mean to you?" <?php if (isset($meta_data['qs']) && ! empty($meta_data['qs'])) { ?>value="<?= $meta_data['qs'] ?>" <?php } ?>
                                                        class="form-control">
                                ...
                                <div class="col-lg-12 text-right mt-3 pr-0">
                                    <button type="button" class="btn btn-primary float-right mb-4"
                                        onclick="add_question_data(this)">Save</button>
                                </div>
                            </form>
                        </div>
                    </div>
                    <div id="form_builder" class="tabcontent ">
                        <div class="container">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <button class="btn btn-outline-blue btn-rounded btn-sm" onclick="show_field_row()">Add
                                    Field</button>
                            </div>
                            <div class="row">
                                <div class="col-md-12 mb-5 pt-3"
                                    style="display:none;border:3px solid grey;border-radius:5px" id="field_row">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <label>Label</label>
                                        ...
                                        <div class="col-md-12">
                                            <button type="button"
                                                class="btn btn-outline-yellow btn-rounded btn-sm float-right mb-3"
                                                onclick="add_field(this)" data-id="billing_info">Add To Billing
                                                Info</button>
                                            <button type="button"
                                                class="btn btn-outline-blue btn-rounded btn-sm float-right mb-3"
                                                onclick="add_field(this)" data-id="personal_info">Add To Personal
                                                Info</button>
                                            <button type="button"
                                                class="btn btn-primary btn-rounded btn-sm float-right mb-3"
                                                onclick="add_field_def(this)" data-id="vasc_id">Vasc Id</button>
                                            <button type="button"
                                                class="btn btn-primary btn-rounded btn-sm float-right mb-3"
                                                onclick="add_field_def(this)" data-id="document_file">Document</button>
                                            <button type="button"
                                                class="btn btn-primary btn-rounded btn-sm float-right mb-3"
                                                onclick="add_field_def(this)" data-id="comment">Invoice Comment</button>
                                            <button type="button"
                                                class="btn btn-primary btn-rounded btn-sm float-right mb-3"
                                                onclick="add_field_def(this)" data-id="note">Note</button>
                                        </div>
                                    </div>
                                </div>
                                <form action="#" method="post" id="order_form">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <select id="c_customer" name="c_customer" class="form-control filter-select"
                                                onchange="get_form_of(this),initial_sortable()"
                                                data-type="copy_customer">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select id="service_types" name="service_type"
                                                class="form-control filter-select"
                                                onchange="get_form_of(this),initial_sortable()" data-type="serv_type">
                                                <?php if ($cus_services) { ?>
                                                    <?php foreach ($cus_services as $cus_ser) { ?>
                                                        <option value="<?= $cus_ser->id ?>"><?= $cus_ser->title ?></option>
                                                    <?php } ?>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select id="copy_from" class="form-control"
                                                onchange="get_form_of(this),initial_sortable()" data-type="copy_from">
                                                <option value=""></option>
                                                <?php if ($all_services) { ?>
                                    ...
                                    <div class="row p-3 mt-4 ml-1 mr-1" id="add_columns_row"
                                        style="border:3px solid grey;border-radius:5px">
                                        <?php if ($default_form) { ?>
            ...
            $form_builder = json_decode($default_form->form);
                                                ...
                                                <div class="row" id="personal_info_row">
                                                    ...
                                                    <?php if (! empty($personal)) { ?>
                                                        <?php foreach ($personal as $input_k => $input_v) { ?>
                                                            ...
                                                            $default_value = isset($real_dta[7]) ? $real_dta[7] : '';
                                                            $unique_id = uniqid();
                                                            $placeholder = htmlspecialchars($placehol);
                                                            $valueToShow = ! empty($default_value) ? htmlspecialchars($default_value) : '';
                                                            ?>
                                                            <?php if ($type != 'radio') { ?>
                                                                <div class="col-md-12 mb-2 sortable-items">
                                                                    <label ondblclick="change_type(this)"
                                                                        onblur="change_label(this)"><?= $label ?><?php if (! empty($req)) { ?><span
                                                                            class="star text-danger">*</span><?php } ?></label>
                                                                    <?php if (empty($is_tra)) { ?><button type="button"
                                                                            class="btn btn-sm float-right <?php if (! empty($req)) { ?> btn-warning <?php } ?>"
                                                                            onclick="change_required(this)"><span
                                                                                class="bi bi-star"></span></button><?php } ?>
                                                                    <?php if (empty($is_tra)) { ?><button type="button"
                                                                            class="btn btn-danger btn-sm float-right"
                                                                            onclick="remove_field(this)"><span
                                                                                class="bi bi-trash"></span></button><?php } ?>
                                                                    <!-- <button type="button" class="btn btn-sm float-right " onclick="show_edit_container(this)"><span class="bi bi-pen"></span></button> -->
                                                                    <input type="<?= $type ?>" class="form-control default_val_inp"
                                                                        name="form_builder[personal_info][<?= $type ?>,<?= $label ?>,<?= $name ?>,<?= $placehol ?>,<?= $req ?>,<?= $is_tra ?>,<?= $is_new ?>,<?= $valueToShow ?> ]"
                                                                        <?php if (isset($input_v) && $input_v !== ''): ?>
                                                                        value="<?= $input_v ?>" <?php endif; ?> data-id="<?= $name ?>">
                                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                                        style="display:none;border:3px solid grey;border-radius:5px">
                                                                        <label for=""><?= $label ?> Default Value</label>
                                                                        <textarea id="default_value_<?= $unique_id ?>"
                                                                            class="form-control mb-4"
                                                                            oninput="updateDefaultValue(this)"><?= $valueToShow ?></textarea>
                                                                    </div>
                                                                </div>
                                                            <?php } else { ?>
                                                                <div class="col-md-12 mb-2 sortable-items">
                                                                    <label ondblclick="change_type(this)"
                                                                        onblur="change_label(this)"></label>
                                                                    <button type="button" class="btn btn-danger btn-sm float-right"
                                                                        onclick="remove_field(this)"><span
                                                                            class="bi bi-trash"></span></button>
                                                                    <button type="button" class="btn btn-success btn-sm float-right"
                                                                        onclick="add_radio_option(this)"
                                                                        data-id="` + builder_type + `"><span
                                                                            class="bi bi-plus"></span></button>
                                                                    <input type="text" class="form-control"
                                                                        name="form_builder[` + builder_type + `][` + type + `,` + label + `,` + label + `,,,,new_field]">
                                                                    <div class="row radio_options">
                                                                        <div class="col-md-12">
                                                                            <div class="row mt-2">
                                                                                <div class="col-md-10">
                                                                                    <input type="text" class="form-control"
                                                                                        name="form_builder[` + builder_type + `][` + label + `][]"
                                                                                        value="Yes">
                                                                                </div>
                                                                                <div class="col-md-2">
                                                                                    <button type="button" class="btn btn-danger float-right"
                                                                                        onclick="delete_opt(this)"><span
                                                                                            class="bi bi-trash"></span></button>
                                                                                </div>
                                                                            </div>
                                                                            <div class="row mt-2">
                                                ...
                                                <div class="row mt-5" id="billing_info_row">
                                                    ...
                                                    <?php if (! empty($billing)) { ?>
                                                        <?php foreach ($billing as $input_b => $input_b) { ?>
                                                            ...
                                                            $b_default_value = isset($b_real_dta[7]) ? $b_real_dta[7] : '';
                                                            $b_unique_id = uniqid();
                                                            $b_placeholder = htmlspecialchars($placehol); // Ensure $placehol is HTML safe
                                                            $b_valueToShow = ! empty($default_value) ? htmlspecialchars($default_value) : '';
                                                            ?>
                                                            <div class="col-md-12 mb-2 sortable-items">
                                                                <label ondblclick="change_type(this)"
                                                                    onblur="change_label(this)"><?= $b_label ?><?php if (! empty($b_req)) { ?><span
                                                                        class="star text-danger">*</span><?php } ?></label>
                                                                <?php if (empty($b_is_tra)) { ?><button type="button"
                                                                        class="btn btn-danger btn-sm float-right"
                                                                        onclick="remove_field(this)"><span
                                                                            class="bi bi-trash"></span></button><?php } ?>
                                                                <!-- <button type="button" class="btn btn-sm float-right" onclick="show_edit_container(this)"><span class="bi bi-pen"></span></button> -->
                                                                <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                                    style="display:none;border:3px solid grey;border-radius:5px"
                                                                    id="edit_container_<?= $b_unique_id ?>">
                                                                    <label for=""><?= $b_label ?> Default Value</label>
                                                                    <textarea id="default_value_<?= $b_unique_id ?>"
                                                                        class="form-control mb-4"
                                                                        oninput="updateDefaultValue(this.value, 'input_field_<?= $b_unique_id ?>')"><?= isset($default_value) ? trim($default_value) : '' ?></textarea>
                                                                </div>
                                                                <?php if (empty($b_is_tra)) { ?><button type="button"
                                                                        class="btn btn-sm float-right <?php if (! empty($b_req)) { ?> btn-warning <?php } ?>"
                                                                        onclick="change_required(this)"><span
                                                                            class="bi bi-star"></span></button><?php } ?>
                                                                <input type="<?= $b_type ?>" id="input_field_<?= $b_unique_id ?>"
                                                                    class="form-control"
                                                                    name="form_builder[billing_info][<?= $b_type ?>,<?= $b_label ?>,<?= $b_name ?>,<?= $b_placehol ?>,<?= $b_req ?>,<?= $b_is_tra ?>,<?= $b_is_new ?>, <?= $b_default_value ?>]"
                                                                    <?php if (isset($b_default_value) && $b_default_value !== ''): ?>
                                                                    value="<?= trim($b_default_value) ?>" <?php else: ?>
                                                                    placeholder="<?= trim($b_placeholder) ?>" <?php endif; ?>
                                                                    data-id="<?= $b_name ?>">
                                                            </div>
                                                        <?php } ?>
                                                    <?php } ?>
                                        ...
                                        <?php } else {
                                            ...
                                            <div class="row" id="personal_info_row">
                                                <h5 style="background-color: lightgrey;">Personal Info</h5>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    <label ondblclick="change_type(this)" onblur="change_label(this)">
                                                        Name<span class="star text-danger">*</span></label>
                                                    <!-- <button type="button" class="btn btn-sm float-right edit-btn" onclick="show_edit_container(this)"><span class="bi bi-pen"></span></button> -->
                                                    <input type="text"
                                                        name="form_builder[personal_info][text,Name,name,Enter Candidate Name,required,n_trash]"
                                                        value="Enter Candidate Name" placeholder="Enter Candidate Name"
                                                        class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                        style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">Name Default Value</label>
                                                        <textarea class="form-control mb-4"
                                                            oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
                                                </div>
                                                <div class="col-md-12 mb-2 sortable-items editable-container ">
                                                    <label ondblclick="change_type(this)"
                                                        onblur="change_label(this)">Surname<span
                                                            class="star text-danger">*</span></label>
                                                    <!-- <button type="button" class="btn btn-sm float-right edit-btn" onclick="show_edit_container(this)"><span class="bi bi-pen"></span></button> -->
                                                    <input type="text"
                                                        name="form_builder[personal_info][text,Surname,surname,Enter Candidate Surname,required,n_trash]"
                                                        value="Enter Candidate Surname"
                                                        placeholder="Enter Candidate Surname"
                                                        class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                        style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">Surname Default Value</label>
                                                        <textarea class="form-control mb-4"
                                                            oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
                                                </div>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    <label ondblclick="change_type(this)"
                                                        onblur="change_label(this)">Email<span
                                                            class="star text-danger">*</span></label>
                                                    <!-- <button type="button" class="btn btn-sm float-right" onclick="show_edit_container(this)"><span class="bi bi-pen"></span></button> -->
                                                    <input type="email"
                                                        name="form_builder[personal_info][email,Email,email,Enter Candidate Email,required,n_trash]"
                                                        value="Enter Candidate Email" placeholder="Enter Candidate Email"
                                                        class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                        style="display:none;border:3px solid grey;border-radius:5px"
                                                        id="edit_container_<?= $s_unique_id ?>">
                                                        <label for="">Email Default Value</label>
                                                        <textarea class="form-control mb-4"
                                                            oninput="updateDefaultValue(this)"><?= isset($default_value) ? $default_value : '' ?></textarea>
                                                    </div>
                                                </div>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    <label ondblclick="change_type(this)"
                                                        onblur="change_label(this)">Phone<span
                                                            class="star text-danger">*</span></label>
                                                    <!-- <button type="button" class="btn btn-sm float-right" onclick="show_edit_container(this)"><span class="bi bi-pen"></span></button> -->
                                                    <input type="text"
                                                        name="form_builder[personal_info][text,Phone,phone,Enter Candidate Phone Number,required,n_trash]"
                                                        value="Enter Candidate Phone Number"
                                                        placeholder="Enter Candidate Phone Number"
                                                        class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                        style="display:none;border:3px solid grey;border-radius:5px"
                                                        id="edit_container_<?= $s_unique_id ?>">
                                                        <label for="">Phone Default Value</label>
                                                        <textarea class="form-control mb-4"
                                                            oninput="updateDefaultValue(this)"><?= isset($default_value) ? $default_value : '' ?></textarea>
                                                    </div>
                                                </div>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    <label ondblclick="change_type(this)" onblur="change_label(this)">Social
                                                        Security Number<span class="star text-danger">*</span></label>
                                                    <!-- <button type="button" class="btn btn-sm float-right" onclick="show_edit_container(this)"><span class="bi bi-pen"></span></button> -->
                                                    <input type="text"
                                                        name="form_builder[personal_info][text,Social Security Number,security,Enter Candidate Social Security Number,required,n_trash]"
                                                        value="Enter Candidate Social Security Number"
                                                        placeholder="Enter Candidate Social Security Number"
                                                        class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                        style="display:none;border:3px solid grey;border-radius:5px"
                                                        id="edit_container_<?= $s_unique_id ?>">
                                                        <label for="">SSN Default Value</label>
                                                        <textarea class="form-control mb-4"
                                                            oninput="updateDefaultValue(this)"><?= isset($default_value) ? $default_value : '' ?></textarea>
                                                    </div>
                                                </div>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    <label ondblclick="change_type(this)" onblur="change_label(this)">VASC
                                                        ID</label>
                                                    <button type="button" class="btn btn-danger btn-sm float-right"
                                                        onclick="remove_field(this)"><span
                                                            class="bi bi-trash"></span></button>
                                                    ...
                                                    <button type="button" class="btn btn-sm float-right"
                                                        onclick="change_required(this)"><span
                                                            class="bi bi-star"></span></button>
                                                    <input type="text" class="form-control default_val_inp"
                                                        name="form_builder[personal_info][text,VASC ID,vasc_id,Enter Candidate VASC ID,]"
                                                        data-id="vasc_id" value="Enter Candidate VASC ID"
                                                        placeholder="Enter Candidate VASC ID">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                        style="display:none;border:3px solid grey;border-radius:5px"
                                                        id="edit_container_<?= $s_unique_id ?>">
                                                        <label for="">VASIC ID Default Value</label>
                                                        <textarea class="form-control mb-4"
                                                            oninput="updateDefaultValue(this)"><?= isset($default_value) ? $default_value : '' ?></textarea>
                                                    </div>
                                                </div>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    <label ondblclick="change_type(this)"
                                                        onblur="change_label(this)">Documents</label>
                                                    <button type="button" class="btn btn-danger btn-sm float-right"
                                                        onclick="remove_field(this)"><span
                                                            class="bi bi-trash"></span></button>
                                                    <button type="button" class="btn btn-sm float-right"
                                                        onclick="change_required(this)"><span
                                                            class="bi bi-star"></span></button>
                                                    <input type="text" class="form-control"
                                                        name="form_builder[personal_info][,Documents,document_file,,,]"
                                                        data-id="document_file">
                                                </div>
                                            </div>
                                            <div class="row mt-5" id="billing_info_row">
                                                <h5 style="background-color: lightgrey;">Billing Info</h5>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    <label ondblclick="change_type(this)"
                                                        onblur="change_label(this)">Reference<br>(Invoice Recipient)<span
                                                            class="star text-danger">*</span></label>
                                                    <!-- <button type="button" class="btn btn-sm float-right" onclick="show_edit_container(this)"><span class="bi bi-pen"></span></button> -->
                                                    <input type="text"
                                                        name="form_builder[billing_info][text,Reference<br>(Invoice Recipient),pref,Enter Candidate Reference,required,n_trash]"
                                                        value="Enter Candidate Reference"
                                                        placeholder="Enter Candidate Reference"
                                                        class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                        style="display:none;border:3px solid grey;border-radius:5px"
                                                        id="edit_container_<?= $s_unique_id ?>">
                                                        <label for="">Reference (Invoice Recipient) Default Value</label>
                                                        <textarea class="form-control mb-4"
                                                            oninput="updateDefaultValue(this)"><?= isset($default_value) ? $default_value : '' ?></textarea>
                                                    </div>
                                                </div>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    <label ondblclick="change_type(this)"
                                                        onblur="change_label(this)">Reference<span
                                                            class="star text-danger">*</span></label>
                                                    <!-- <button type="button" class="btn btn-sm float-right" onclick="show_edit_container(this)"><span class="bi bi-pen"></span></button> -->
                                                    <input type="text"
                                                        name="form_builder[billing_info][text,Reference,ref,Enter Candidate Reference,required,n_trash]"
                                                        placeholder="Enter Candidate Reference"
                                                        value="Enter Candidate Reference"
                                                        class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                        style="display:none;border:3px solid grey;border-radius:5px"
                                                        id="edit_container_<?= $s_unique_id ?>">
                                                        <label for="">Reference Default Value</label>
                                                        <textarea class="form-control mb-4"
                                                            oninput="updateDefaultValue(this)"><?= isset($default_value) ? $default_value : '' ?></textarea>
                                                    </div>
                                                </div>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    <label ondblclick="change_type(this)"
                                                        onblur="change_label(this)">Invoice Comment</label>
                                                    <button type="button" class="btn btn-danger btn-sm float-right"
                                                        onclick="remove_field(this)"><span
                                                            class="bi bi-trash"></span></button>
                                                    <!-- <button type="button" class="btn btn-sm float-right" onclick="show_edit_container(this)"><span class="bi bi-pen"></span></button> -->
                                                    <button type="button" class="btn btn-sm float-right"
                                                        onclick="change_required(this)"><span
                                                            class="bi bi-star"></span></button>
                                                    <input type="text" class="form-control default_val_inp"
                                                        name="form_builder[billing_info][text,Invoice Comment,comment,Enter Invoice Comment,]"
                                                        data-id="comment" value="Enter Invoice Comment"
                                                        placeholder="Enter Invoice Comment">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                        style="display:none;border:3px solid grey;border-radius:5px"
                                                        id="edit_container_<?= $s_unique_id ?>">
                                                        <label for="">Invoice Comment Default Value</label>
                                                        <textarea class="form-control mb-4"
                                                            oninput="updateDefaultValue(this)"><?= isset($default_value) ? $default_value : '' ?></textarea>
                                                    </div>
                                                </div>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    <label ondblclick="change_type(this)"
                                                        onblur="change_label(this)">Note</label>
                                                    <button type="button" class="btn btn-danger btn-sm float-right"
                                                        onclick="remove_field(this)"><span
                                                            class="bi bi-trash"></span></button>
                                                    <!-- <button type="button" class="btn btn-sm float-right" onclick="show_edit_container(this)"><span class="bi bi-pen"></span></button> -->
                                                    <button type="button" class="btn btn-sm float-right"
                                                        onclick="change_required(this)"><span
                                                            class="bi bi-star"></span></button>
                                                    <input type="text" class="form-control default_val_inp" data-id="note"
                                                        name="form_builder[billing_info][text,Note,note,Here you can create a note about the order that is visible to both you and us. Please note that this note will not be visible to the individual.,]"
                                                    ...
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container"
                                                        style="display:none;border:3px solid grey;border-radius:5px"
                                                        id="edit_container_<?= $s_unique_id ?>">
                                                        <label for="">Note Default Value</label>
                                                        <textarea class="form-control mb-4"
                                                            oninput="updateDefaultValue(this)"><?= isset($default_value) ? $default_value : '' ?></textarea>
                                                    </div>
                                                </div>
                                    ...
                                    <div class="col-lg-12 text-right mt-3 pr-0">
                                        <button type="button" class="btn btn-primary float-right mb-4"
                                            onclick="save_form_builder(this)">Save</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                                ...
                                <div class="col-lg-12">
                                    <div class="table-section">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            ...
                                            <button class="btn btn-outline-blue btn-rounded btn-sm"
                                                onclick="openCity(event, 'add_additional_customers')">Add</button>
                                        </div>
                                        <table class="table table-bordered">
                                            <thead>
                                                <tr>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td style="width:6% !important">
                                                                <div class="dropdown">
                                                                    ...
                                                                    <ul class="dropdown-menu p-2 ps-3 table-menu-btn-list">
                                                                        <li class="mb-1"><a href="#"
                                                                                onclick="openCity(event, 'update_additional_customers'),get_ad_cu_data(this)"
                                                                                data-id="<?= $ad_cu->id ?>"
                                                                                data-name="<?= $ad_cu->name ?>"
                                                                                data-email="<?= $ad_cu->email ?>"
                                                                                class="no-decoration f-14 w-600 text-black "><i
                                                                                    class="bi bi-pen text-black f-14 me-2"></i>
                                                                                Edit</a>
                                                                        </li>
                                                                        <li class="mb-1"><a href="#"
                                                                                class="no-decoration f-14 w-600 text-black"
                                                                                onclick="delete_ad_cu(this)"
                                                                                data-id="<?= $ad_cu->id ?>"><i
                                                                                    class="bi bi-trash  f-14 text-black me-2"></i>
                                                                                Delete</a>
                                                                        </li>
                                                                    </ul>
                                                                </div>
                    ...
                    <div id="service_cost" class="tabcontent ">
                        <div class="container">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <label>Service Type</label>
                                    <select id="service_type" name="service_type" class="form-control filter-select"
                                        onchange="handleServiceChange(this)" data-type="serv_type">
                                        <?php if ($cus_services) { ?>
                                            <?php foreach ($cus_services as $cus_ser) { ?>
                                                <option value="<?= $cus_ser->id ?>"><?= $cus_ser->title ?></option>
                                            <?php } ?>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" id="service_value" value="" placeholder="Service Cost" required
                                        class="w-100 from-input">
                                </div>
                            </div>
                            <button type="button"
                                class="btn btn-primary float-right mb-4 mt-4 update_cost">Save</button>
                        </div>
                        <p class="text-success text-center mt-3" hidden>Service Cost has Updated Successfully!</p>
                    </div>
                                ...
                                <div class="col-lg-12">
                                    <div class="table-section">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            ...
                                            <button class="btn btn-outline-blue btn-rounded btn-sm back_btn"
                                                onclick="openCity($('#addCuOpen'), 'additional_customers' )">Back</button>
                                        </div>
                                        <div class="d-flex align-items-center form-row mb-3">
                                            <label for="" class="label-lg">Name</label>
                                            <input type="text" name="add_cus_name"
                                                placeholder="Additional Customer name" required
                                                class="w-100 from-input">
                                            <input type="hidden" name="add_cus_id" value="<?= $_GET['id'] ?>">
                                        </div>
                                        <div class="d-flex align-items-center form-row mb-3">
                                            <label for="" class="label-lg">Email</label>
                                            <input type="email" name="add_cus_email"
                                                placeholder="Additional Customer email" required
                                                class="w-100 from-input">
                                        </div>
                                        <div class="d-flex justify-content-end">
                                            <button type="button" class="btn btn-outline-blue btn-rounded btn-sm"
                                                id="add_ad_cu_btn" onclick="additional_customer_data(this)">Add</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-lg-12">
                                    <div class="table-section">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            ...
                                            <button class="btn btn-outline-blue btn-rounded btn-sm back_btn"
                                                onclick="openCity($('#adCuOpen'), 'additional_customers' )">Back</button>
                                        </div>
                                        <div class="d-flex align-items-center form-row mb-3">
                                            <label for="" class="label-lg">Name</label>
                                            <input type="text" name="add_cus_name_u"
                                                placeholder="Additiona customer name" required class="w-100 from-input">
                                            <input type="hidden" name="add_cus_id_u">
                                        </div>
                                        <div class="d-flex align-items-center form-row mb-3">
                                            <label for="" class="label-lg">Email</label>
                                            <input type="email" name="add_cus_email_u"
                                                placeholder="Additiona customer email" required
                                                class="w-100 from-input">
                                        </div>
                                        <div class="d-flex justify-content-end">
                                            <button type="button" class="btn btn-outline-blue btn-rounded btn-sm"
                                                id="update_ad_cu_btn"
                                                onclick="additional_customer_data(this)">Update</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    (function() {
        ...
        let $company = $('#company');
        let $suggestions = $('#company_suggestions');
        // If #company isn't the expected visible text input (might be duplicated elsewhere), try fallback
        if ($company.length === 0 || ($company.prop && $company.prop('tagName') !== 'INPUT') || ($company.attr && $company.attr('type') && $company.attr('type') !== 'text')) {
            var $alt = $('form.update-form').find('input[name="company"]:visible').first();
            if ($alt.length) {
                $company = $alt;
                console.log('Fallback found company input via form selector:', $company.length);
            } else {
                console.warn('No fallback company input found. Autocomplete will not initialize.');
            }
        }
        if ($company && $company.length) {
            console.log('Company element tag/type:', $company.prop('tagName'), $company.attr('type'));
        }
    ...
    function show_edit_container(obj) {
        $('.edit-container').slideUp(500);
        if ($(obj).closest('div').find('.edit-container').is(':visible')) {
            $(obj).closest('div').find('.edit-container').slideUp(500);
        } else {
            $(obj).closest('div').find('.edit-container').slideToggle(500);
        }
    }
    ...
    function change_manager_company(cus_id) {
        ...
        var email_temp_approved = $('#email_template_approved').val();
        ...
        $.ajax({
            ...
            data: {
                ...
                email_temp_approved: email_temp_approved,
                ...
                if (response != '') {
                    ...
                    var personal_info = `<h5 style="background-color: lightgrey;">Personal Info</h5>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    ...
                                                    <input type="text" name="form_builder[personal_info][text,Name,name,Enter Candidate Name,required,n_trash]" value="Enter Candidate Name" placeholder="Enter Candidate Name" class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container" style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">Name Default Value</label>
                                                        <textarea class="form-control mb-4" oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
                                                </div>
                                                <div class="col-md-12 mb-2 sortable-items editable-container ">
                                                    <label ondblclick="change_type(this)" onblur="change_label(this)">Surname<span class="star text-danger">*</span></label>
                                                    <input type="text" name="form_builder[personal_info][text,Surname,surname,Enter Candidate Surname,required,n_trash]" value="Enter Candidate Surname" placeholder="Enter Candidate Surname" class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container" style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">Surname Default Value</label>
                                                        <textarea class="form-control mb-4" oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
                                                ...
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    ...
                                                    <input type="email" name="form_builder[personal_info][email,Email,email,Enter Candidate Email,required,n_trash]" value="Enter Candidate Email" placeholder="Enter Candidate Email" class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container" style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">Email Default Value</label>
                                                        <textarea class="form-control mb-4" oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
                                                ...
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    ...
                                                    <input type="text" name="form_builder[personal_info][text,Phone,phone,Enter Candidate Phone Number,required,n_trash]" value="Enter Candidate Phone Number" placeholder="Enter Candidate Phone Number" class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container" style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">Phone Default Value</label>
                                                        <textarea class="form-control mb-4" oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
                                                ...
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    ...
                                                    <input type="text" name="form_builder[personal_info][text,Social Security Number,security,Enter Candidate Social Security Number,required,n_trash]" value="Enter Candidate Social Security Number" placeholder="Enter Candidate Social Security Number" class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container" style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">SSN Default Value</label>
                                                        <textarea class="form-control mb-4" oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
                                                ...
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    ...
                                                    <button type="button" class="btn btn-danger btn-sm float-right" onclick="remove_field(this)"><span class="bi bi-trash"></span></button>
                                                    <button type="button" class="btn btn-sm float-right" onclick="change_required(this)"><span class="bi bi-star"></span></button>
                                                    <input type="text" class="form-control default_val_inp" name="form_builder[personal_info][text,VASC ID,vasc_id,Enter Candidate VASC ID,]" data-id="vasc_id" value="Enter Candidate VASC ID" placeholder="Enter Candidate VASC ID">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container" style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">VASIC ID Default Value</label>
                                                        <textarea class="form-control mb-4" oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
                    ...
                    var billing_info = `<h5 style="background-color: lightgrey;">Billing Info</h5>
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    ...
                                                    <input type="text" name="form_builder[billing_info][text,Reference<br>(Invoice Recipient),pref,Enter Candidate Reference,required,n_trash]" value="Enter Candidate Reference" placeholder="Enter Candidate Reference" class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container" style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">Reference (Invoice Recipient) Default Value</label>
                                                        <textarea class="form-control mb-4" oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
                                                ...
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    ...
                                                    <input type="text" name="form_builder[billing_info][text,Reference,ref,Enter Candidate Reference,required,n_trash]" placeholder="Enter Candidate Reference" value="Enter Candidate Reference" class="form-control default_val_inp">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container" style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">Reference Default Value</label>
                                                        <textarea class="form-control mb-4" oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
                                                ...
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    ...
                                                    <button type="button" class="btn btn-danger btn-sm float-right" onclick="remove_field(this)"><span class="bi bi-trash"></span></button>
                                                    <button type="button" class="btn btn-sm float-right" onclick="change_required(this)"><span class="bi bi-star"></span></button>
                                                    <input type="text" class="form-control default_val_inp" name="form_builder[billing_info][text,Invoice Comment,comment,Enter Invoice Comment,]" data-id="comment" value="Enter Invoice Comment" placeholder="Enter Invoice Comment">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container" style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">Invoice Comment Default Value</label>
                                                        <textarea class="form-control mb-4" oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
                                                ...
                                                <div class="col-md-12 mb-2 sortable-items editable-container">
                                                    ...
                                                    <button type="button" class="btn btn-danger btn-sm float-right" onclick="remove_field(this)"><span class="bi bi-trash"></span></button>
                                                    <button type="button" class="btn btn-sm float-right" onclick="change_required(this)"><span class="bi bi-star"></span></button>
                                                    <input type="text" class="form-control default_val_inp" data-id="note" name="form_builder[billing_info][text,Note,note,Here you can create a note about the order that is visible to both you and us. Please note that this note will not be visible to the individual.,]" value="Here you can create a note about the order that is visible to both you and us. Please note that this note will not be visible to the individual." placeholder="Here you can create a note about the order that is visible to both you and us. Please note that this note will not be visible to the individual.">
                                                    <div class="col-md-12 mb-5 pt-3 mt-4 edit-container" style="display:none;border:3px solid grey;border-radius:5px">
                                                        <label for="">Note Default Value</label>
                                                        <textarea class="form-control mb-4" oninput="updateDefaultValue(this)"></textarea>
                                                    </div>
    ...
    function updateDefaultValue(obj) {
        const inputField = $(obj).closest('div').parent().find('.default_val_inp');
        let value = $(obj).val();
        if (!inputField) {
            return;
        }
        let nameAttr = inputField.attr('name');
        if (!nameAttr) {
            return;
        }
        let nameParts = nameAttr.split(',');
...
<script>
    ...
    $("#update_customer_btn").on("click", function(e) {
        ...
        if (selectedServices.length > 0) {
            servicesString = selectedServices.map(function() {
                return this.value;
            }).get().join(',');
        }
        ...
        if (selectedStatuses.length > 0) {
            statusesString = selectedStatuses.map(function() {
                return this.value;
            }).get().join(',');
        }
    ...
    // })
    // Resend Email - Fixed to prevent multiple event listeners
    // First, remove any existing event listeners
    $(document).off("click", ".resend_btn");
    // Then attach the new event listener
    ...
    $('#message_customer').on('change', function() {
        var sid = $('#copy_from_messages').val();
        var cus_id = $(this).val();
        var formData = new FormData($(this).closest("form")[0]);
        if (cus_id) {
    ...
    $('#copy_from_messages').on('change', function() {
        var cus_id = $('#message_customer').val();
        var sid = $(this).val();
        var formData = new FormData($(this).closest("form")[0]);
        formData.append('type', 'fetch_messages_cus');
        formData.append('id', id);
    ...
    $('.update_cost').click(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                let successMessage = $('.text-success');
