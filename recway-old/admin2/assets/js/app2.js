...
$(document).ready(function () {
    ...
    if ($('#dataTable').attr('data-table') === 'candidate') {
        ...
        table = $('#dataTable').DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    ...
                    // Read delivery date values - check if elements exist
                    var deliveryFromEl = $('#delivery_date_from');
                    var deliveryToEl = $('#delivery_date_to');
                    d.delivery_date_from = deliveryFromEl.length > 0 ? deliveryFromEl.val() || '' : '';
                    d.delivery_date_to = deliveryToEl.length > 0 ? deliveryToEl.val() || '' : '';
            ...
            drawCallback: function (settings) {
                ...
                $('.bg-check-alert-icon').off('mouseenter.bgcheck mouseleave.bgcheck');
                // Initialize tooltips for background check alert icons
                $('.bg-check-alert-icon').each(function () {
                    var $icon = $(this);
                    var orderId = $icon.attr('data-order-id');
                    if (!orderId) {
                        return;
                    }
                ...
                $('.bg-check-alert-icon').on('mouseenter.bgcheck', function (e) {
                    ...
                    // Mark as loading
                    $icon.data('tooltip-loading', true);
                    // Make AJAX request
                    var ajaxRequest = $.ajax({
                        ...
                        success: function (response) {
                            ...
                            $icon.removeData('tooltip-loading');
                            if (response.success && response.tooltip_html) {
                    ...
                    var $icon = $(this);
                    var tooltip = $icon.data('tooltip-instance') || bootstrap.Tooltip.getInstance(this);
        ...
        table.on('init.dt', function () { wireDeferredSearch(table); });
    } else if ($('#dataTable').attr('data-table') === 'statuses') {
        // Statuses DataTable
        var __url = new URL(window.location.href);
        var __serviceCatId = __url.searchParams.get('id') || (typeof service_cat_id !== 'undefined' ? service_cat_id : '');
        table = $('#dataTable').DataTable({
            language: { search: "", searchPlaceholder: "Search..." },
            dom: "<'row'<'col-sm-6'l><'col-sm-6'f>>t<'row'<'col-sm-6'i><'col-sm-6'p>>",
            scrollX: true,
            ...
            ajax: {
                ...
                type: 'POST',
                data: function (d) {
                    d.action = 'get_statuses_data';
                    d.service_cat_id = __serviceCatId;
                    return d;
                },
                complete: function () {
                    // Initialize dropdowns after data loads
                    setTimeout(function () {
                        $('.dropdownBtn').each(function () {
                            if (!$(this).data('bs.dropdown')) {
                                new bootstrap.Dropdown($(this)[0]);
            ...
            drawCallback: function (settings) {
                // Reinitialize dropdowns on each draw
                setTimeout(function () {
                    $('.dropdownBtn').each(function () {
                        if (!$(this).data('bs.dropdown')) {
                            new bootstrap.Dropdown($(this)[0]);
        ...
        table.on('init.dt', function () { wireDeferredSearch(table); });
    } else if ($('#dataTable').attr('data-table') === 'news_reports') {
        table = $('#dataTable').DataTable({
            language: { search: "", searchPlaceholder: "Search..." },
            dom: "<'row'<'col-sm-6'l><'col-sm-6'f>>t<'row'<'col-sm-6'i><'col-sm-6'p>>",
            scrollX: true,
            order: [[2, 'asc']], // Sort by Title ascending by default
            pageLength: 10,
            processing: true,
            serverSide: true,
            search: false,
            columnDefs: [
                { orderable: false, targets: [0, 1, 5] } // Disable sorting on Action, #, and PDF columns
            ],
            ajax: {
                url: '../includes/pages.php',
                type: 'POST',
                data: function (d) {
                    d.action = 'get_news_reports_data';
                    return d;
                },
                complete: function () {
                    $('.dropdownBtn').each(function () {
    ...
    if (name != '' && message != '') {
        $.ajax({
            ...
            success: function (response) {
                ...
                alert('Custom message added successfully')
            },
            error: function () {
                alert('Error adding custom message')
            }
        });
    } else {
...
function update_custom_message() {
    var name = $('#main_u_name').val();
    var message = $('#main_u_message').val();
    var id = $('#main_u_id').val();
    if (name != '' && message != '') {
        $.ajax({
            type: "POST",
            url: "./includes/table_ajax.php",
            data: {
                update_custom_message: 1,
                name: name,
                message: message,
                u_id: id
            },
            success: function (response) {
                $('#update_custom_message_card').hide();
                if ($('#dataTable').length && $('#dataTable').DataTable) {
                    $('#dataTable').DataTable().ajax.reload();
                }
            ...
            error: function () {
                alert('Error updating custom message')
            }
        });
    } else {
        alert('Please fill in both name and message fields')
...
function update_s(obj) {
    ...
    var message = $(obj).closest('ul').find('.u_message').val()

    // Check if it's custom messages page
    if ($('#update_custom_message_card').length > 0) {
        if ($('#update_custom_message_card').is(':hidden')) {
            $('#update_custom_message_card').slideDown(500);
            $('#main_u_id').val(id)
            $('#main_u_name').val(name)
            $('#main_u_message').val(message)
        } else {
            $('#update_custom_message_card').slideUp(500);
            $('#main_u_id').val('')
            $('#main_u_name').val('')
            $('#main_u_message').val('')
        }
    } else if ($('#update_service_card').length > 0) {
