                                ...
                                <tbody id="filtertab2">
                                    <?php if (!empty($booked_order_cus)) {
                                        ...
                                        <?php foreach ($booked_order_cus as $booked_order_cu) { ?>
                                            <tr data-comp="<?= trim($uninvoiced_order->company_name) ?>">
                                                <td><?= ++$i ?></td>
                            ...
                            <table class="table-bordered">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th>Company Name</th>
                                                                                <th>Interview / Delivery Date From</th>
                                        <th>Interview / Delivery Date To</th>
                                        <th>No of Orders</th>
                                        <th></th>
                                    </tr>
                                </thead>
                                            ...
                                            <tr>
                                                ...
                                                <td><?= $booked_order_cu->company ?></td>
                                                    ...
                                                    <input type="date" class="form-control s_date"
                                                        ...
                                                        onchange="fetch_data(this,'<?= $booked_order_cu->company ?>')">
                                                </td>
                                                <td class="p-0">
                                                ...
                                                <td>
                                                <a href="pages/orders.php?company=<?= trim($booked_order_cu->company_name) ?>&startDate=<?php echo $endDate ?>&endDate=<?php echo $startDate ?>" data-company="<?= trim($booked_order_cu->company_name) ?>" class="text-dark open-order"><?= $booked_order_cu->order_count ?></a>    
                                                </td>
                                                <td>
                        ...
                        <div class="col-lg-12 d-flex flex-column justify-content-between align-items-between">
                            <div class="">
                                <div class="row bg-light p-3">
                                    ...
                                    <div class="col-md-12">
                                        <div class="d-flex align-items-start">
                                            <input type="checkbox" class="form-check-input" checked value="service__cost" name="customize_columns[]" id="service__cost">
                                            <label style="line-height: 14px" for="service__cost" class="form-check-label ">
                                                Service Cost
                                            </label>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="d-flex align-items-start">
                                            <input type="checkbox" class="form-check-input" checked value="travel__cost" name="customize_columns[]" id="travel__cost">
                                            <label style="line-height: 14px" for="travel__cost" class="form-check-label ">
                                                Travel Cost
                                            </label>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="d-flex align-items-start">
                                            <input type="checkbox" class="form-check-input" checked value="total__cost" name="customize_columns[]" id="total__cost">
                                            <label style="line-height: 14px" for="total__cost" class="form-check-label ">
                                                Total Cost
                                            </label>
                                        </div>
                                    </div>
...
<script>
            ...
            success: function(response) {
                ...
                if (response.uninvoiced_orders && response.uninvoiced_orders != '') {
                    ...
                    response.uninvoiced_orders.forEach(function(order, k) {
                        ...
                        html += `<tr data-id="` + $.trim(order.customer_company) + `">
                                                <td>` + i++ + `</td>
                ...
                if (response.comp_with_orders && response.comp_with_orders != '') {
                    ...
                    response.comp_with_orders.forEach(function(order, k) {
                        html += `<tr>
                                                ...
                                                <td>` + order.company + `</td>
                                                                                                <td class="p-0">
                                                    <input type="date" class="form-control s_date" value="`+ startDate + `" onchange="fetch_data(this,'` + $.trim(order.company) + `')">
                                                </td>
                                                <td class="p-0">
                                                    <input type="date" class="form-control e_date" value="`+ endDate + `" onchange="fetch_data(this,'` + $.trim(order.company) + `')">
                                                </td>
                                                <td><a href="pages/orders.php?company=` + $.trim(order.company) + `&startDate=` + startDate + `&endDate=` + endDate + `" data-company="` + $.trim(order.company) + `" class="text-dark open-order">` + order.order_count + `</a></td>
                                                <td>
    ...
    var oldCompany = $.trim($('.company-select').val());
    var currentCompany = $.trim($(obj).data('name'));

    // Store original global dates
    var originalStart = $('#startDate').val();
    var originalEnd = $('#endDate').val();

    // Set the company in the dropdown
    $('.company-select option').each(function () {
        if ($.trim($(this).val()) === currentCompany) {
    ...
    // Get latest date values from the row
    var $tr = $(obj).closest('tr');
    var sDate = $tr.find('.s_date').val();
    var eDate = $tr.find('.e_date').val();

    // Temporarily set global date filters
    $('#startDate').val(sDate);
    $('#endDate').val(eDate);
    ...
    setTimeout(function () {
        ...
        // Restore company
        $('.company-select option').each(function () {
            if ($.trim($(this).val()) === oldCompany) {
                $(this).prop('selected', true);
            }
        });

        $('.billing_detail').prop('checked', true);

        // Restore original global date filters
        $('#startDate').val(originalStart);
        $('#endDate').val(originalEnd);

        // Optional: Send AJAX if still needed
        $.ajax({
            type: "POST",
            url: '../includes/pages.php',
            data: {
                ...
                "company_exported": 1,
                "interview_date_from": sDate,
                "interview_date_to": eDate,
            },
            success: function (response) {
                if (response != '') {
                    // handle success
    ...
    }, 200); // Allow company select and date updates to apply
}
    ...
    function fetch_data(obj, comp = null) {
        var $tr = $(obj).closest('tr');
        var startDate = $tr.find('input[type="date"]').eq(0).val();
        var endDate = $tr.find('input[type="date"]').eq(1).val();
        var service = $('.service-select').val()
        if (service) {
        } else {
            service = 0
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.comp_with_orders && response.comp_with_orders != '') {
                    ...
                    var j = 1;
                    response.comp_with_orders.forEach(function (order, k) {
                        updateRow(order, response.exported_company, obj, startDate, endDate);
                    });
                }
                $('#un_invo tr[data-comp="' + $.trim(comp) + '"]').remove();
                if (response.uninvoiced_orders && response.uninvoiced_orders != '') {
    var html = '';
    ...
    response.uninvoiced_orders.forEach(function (order, k) {
        ...
        var btn_class = '';
        order.order_history.forEach(function (row) {
            if (row.desc === 'Interview Interrupted' ||
                row.desc === 'Candidate is under investigation with SPO' ||
                row.desc === 'Candidate has been denied after meeting with SPO' ||
                row.desc === 'Interview has been Rescheduling' ||
                row.desc === 'Candidate did not show up') {
                btn_class = 'btn-success';
            }
        });

        html += `<tr data-comp="` + $.trim(order.customer_company) + `">
            <td></td> <!-- We'll update S.No. later -->
            <td>` + order.order_id + `</td>
            <td>` + order.customer_name + `</td>
            <td>` + order.customer_company + `</td>`;
        ...
        html += `<td>
                    <button class="` + btn_class + `" type="button" data-id="` + order.id + `" onclick="get_history(this)" style="float: right; margin: 5px;">
                        <i class="bi bi-clock-history"></i>
                    </button>
                </td>
                <td><span class="badge badge-pill" style="background-color:` + order.status_color + `">` + order.status_name + `</span></td>
                <td class="text-center">
                    <input type="checkbox" class="form-check-input" data-chkcomp="` + $.trim(order.customer_company) + `" value="` + order.order_id + `" id="inv_` + order.order_id + `" onclick="invoice_sent(this)">
                    <label for="inv_` + order.order_id + `" class="form-check-label"></label>
                </td>
            </tr>`;
    });
    ...
    $('#un_invo tr').each(function (index) {
        $(this).find('td:first').text(index + 1);
    });

    $('.u_n_count').text($('#un_invo tr').length);
}

            }
        });
    }
    function updateRow(order, exported_company, obj) {
        let $tr = $(obj).closest('tr');
        // Get current date values from the inputs in this row
        let startDate = $tr.find('input[type="date"]').eq(0).val();
        let endDate = $tr.find('input[type="date"]').eq(1).val();

        // Update order count td
        let orderCountTd = `<td>
        <a href="pages/orders.php?company=${encodeURIComponent($.trim(order.company))}&startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}" 
           data-company="${$.trim(order.company)}" 
           class="text-dark open-order">
            ${order.order_count}
        </a>
    </td>`;

        // Check if company is already exported
        let btnClass = '';
        if (exported_company) {
            exported_company.forEach(function (ex) {
                if ($.trim(ex.exported_company) === $.trim(order.company)) {
        ...
        // Update trigger export button td
        let buttonTd = `<td>
        <button type="button" data-name="${order.company}" class="${btnClass}" 
                onclick="trigger_export(this);$(this).addClass('btn-success')">
            <span class="fa fa-print"></span>
        </button>
    </td>`;
    $tr.find('td').eq(4).empty().append($(orderCountTd).html());
$tr.find('td').eq(6).empty().append($(buttonTd).html());

let newLink = $tr.find('td').eq(4).find('a.open-order');
if (newLink.length) {
    let company = newLink.data('company');
    let startDate = $tr.find('td').eq(2).find('input').val();
    let endDate = $tr.find('td').eq(3).find('input').val();
    
    newLink.attr('href', `pages/orders.php?company=${company}&startDate=${startDate}&endDate=${endDate}`);
...
$("body").on("click", ".open-order", function (e) {

    e.preventDefault();

    const company = $(this).data("company")

    const interview_date_from = $('#startDate').val()

    const interview_date_to = $('#endDate').val()

    // const url = "pages/orders.php?company=" + company + "&startDate=" + interview_date_from + "&endDate=" + interview_date_to;
const url = this.href;
    $("#content-modal .modal-title").text("Orders")
