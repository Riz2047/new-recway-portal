...
<style>
    ...
    #proofImagesModal .modal-body {
        max-height: 70vh;
        overflow-y: auto;
    }
    #proofImagesModal .card {
        border: 1px solid #e9ecef;
        border-radius: 8px;
    }
    #proofImagesModal .card:hover {
        border-color: #007bff;
    }
    .show-proofs-btn {
        font-size: 12px;
                ...
                <table id="dataTable" data-table="candidate" class="display Table" style="width: 100% !important">
                    <thead>
                        <tr>
                            ...
                            <th class="table-head">Identity</th>
                        </tr>
                    </thead>
                    <tbody>
...
<!-- Proof Images Modal -->
<div class="modal fade" id="proofImagesModal" tabindex="-1" aria-labelledby="proofImagesModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="proofImagesModalLabel">Verification Proofs</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div id="proofImagesContainer" class="row">
                    <!-- Images will be loaded here -->
                </div>
                <div id="noProofsMessage" class="text-center text-muted" style="display: none;">
                    <i class="fas fa-images fa-3x mb-3"></i>
                    <p>No verification proofs available for this candidate.</p>
                </div>
            </div>
...
function resentVerification(obj) {
                console.log('Resend Verification button clicked');
                var candidateId = $(obj).data('candidate-id');
                var button = $(obj);
                // Disable button and show loading state
                button.prop('disabled', true);
                button.html('<i class="fas fa-spinner fa-spin"></i>');
                console.log('Sending AJAX request for candidate ID:', candidateId);
                $.ajax({
                    type: "POST",
                    url: "resent-verification.php",
                    data: {
                    ...
                    success: function(response) {
                        ...
                        try {
                            ...
                            if (result.success) {
                                ...
                                button.closest('td').find('small').remove();
                                button.after('<br><small class="text-muted">Last resent: ' + timestamp + '</small>');
                                // Reset button
    ...
    function showProofs(obj) {
        ...
        $('#proofImagesContainer').empty();
        $('#noProofsMessage').hide();
        // Show modal
        var modal = new bootstrap.Modal(document.getElementById('proofImagesModal'));
        modal.show();
        // Update modal title
        $('#proofImagesModalLabel').text('Verification Proofs - Candidate ID: ' + candidateId);
        // Make AJAX request to get proof images
        $.ajax({
            type: "POST",
            url: "../includes/get-proof-images.php", // This endpoint needs to be created
            data: {
                'candidate_id': candidateId
            },
            success: function(response) {
    ...
    if (response.success && response.images && response.images.length > 0) {
        response.images.forEach(function(imagePath, index) {
            ...
            $('#proofImagesContainer').append(imageHtml);
        });
    } else {
        $('#noProofsMessage').show();
    }
},
            error: function(xhr, status, error) {
                console.log('AJAX Error:', status, error);
            ...
            complete: function() {
                // Reset button
                button.prop('disabled', false);
                button.html('<i class="fas fa-images"></i>');
            }
        });
    }
    function openImageModal(imageSrc) {
        // Create a simple image modal for full-size viewing
        var imageModalHtml = '<div class="modal fade" id="imageViewModal" tabindex="-1" aria-hidden="true">' +
            '<div class="modal-dialog modal-xl modal-dialog-centered">' +
            '<div class="modal-content">' +
            '<div class="modal-header">' +
            '<h5 class="modal-title">Verification Proof</h5>' +
            '<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>' +
            '</div>' +
            '<div class="modal-body text-center">' +
            '<img src="' + imageSrc + '" class="img-fluid" alt="Verification Proof">' +
            '</div>' +
            '</div>' +
            '</div>' +
            '</div>';
