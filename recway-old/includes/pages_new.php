...
if (isset($_POST['type']) && $_POST['type'] == "update_candidate") {
    ...
    if (!empty($_FILES['files']['name'][0])) {
        ...
        $query = 'UPDATE candidates SET name = ?, surname = ?, status = ?, email = ?, phone = ?, place = ?, security = ?, vasc_id = ?, note = ?, interview_id = ?, background_check_date = ?, delivery_date = ?, combine_interview_id = ?, cv = ?  WHERE id = ?';
        $stmt = $conn->prepare($query);
        $res = $stmt->execute([$name, $surname, $status, $email, $phone, $place, $security, $vasc_id, $note, $service, $background_check_date, $delivery_date, $combine_interview_id, $files, $_POST['id']]);
    } else {
...
if (isset($_POST['action']) && $_POST['action'] == 'get_candidates_data') {
    ...
    $baseQuery = "SELECT c.*, 
                        ...
                        CASE 
                            ...
                            THEN 
                                CASE 
                                    WHEN DATEDIFF(NOW(), h.date_time) < 28 
                                    THEN CONCAT('After ', 28 - DATEDIFF(NOW(), h.date_time), ' days')
    ...
    // Add interview date filters (uses booked as interview date)
    if (!empty($interview_date_from)) {
    ...
    $columns = [
        ...
        'c.security',              // 8 SSN
        'cu.name',                 // 9 Customer
        'cu.company',              // 10 Company
        's.name',                  // 11 Staff
        'c.reported_to_sm',        // 12 Reported
        'st.status',               // 13 Status
        'c.invoice_sent',          // 14 Invoice Sent
        'c.booked',                // 15 Interview Date
        'c.economy',               // 16 Economy
        'c.criminal_record',       // 17 Criminal Record
        'c.social',                // 18 Social Media
        'c.invoice_date',          // 19 Invoice Date
        'c.background_check_date', // 20 Background Check Date
        'c.created',               // 21 Order Created
        'days_to_archive',         // 22 Archive Time (calculated alias)
        'c.delivery_date',         // 23 Delivery Date
        'i.title',                 // 24 Service Type
        'c.is_verified'            // 25 Identity
    ];
    ...
    foreach ($candidates as $index => $candidate) {
        ...
        $row = [
            ...
            // $deliveryDate = $candidate['delivery_date'] ?? 'N/A',
...
if (isset($_POST['action']) && $_POST['action'] == 'export_candidates_excel_admin') {
    ...
    if (!empty($interview_date_from)) { $baseQuery .= " AND DATE(c.booked) >= ?"; $params[] = $interview_date_from; $paramTypes[] = PDO::PARAM_STR; }
    if (!empty($interview_date_to))   { $baseQuery .= " AND DATE(c.booked) <= ?"; $params[] = $interview_date_to;   $paramTypes[] = PDO::PARAM_STR; }
    if (!empty($fil_status)) {
        $baseQuery .= " AND c.status = ?";
...
if (isset($_POST['action']) && $_POST['action'] == 'export_candidates_excel') {
    ...
    $baseQuery = "SELECT c.*, 
                  ...
                  AND NOT (
                    ...
                    AND DATEDIFF(NOW(), h.date_time) >= 28
                  )";
    $params = [];
    $paramTypes = [];
    if (!empty($service) && $service != 'all') {
