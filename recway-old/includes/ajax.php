...
// Admin Updates - List
if (isset($_POST['updates_list_admin'])) {
    global $pdo;
    $conn = $pdo->open();
    try {
        $query = "SELECT id, title_en, content_en, title_swg, content_swg, visible, DATE_FORMAT(created_at, '%Y-%m-%d %H:%i') AS created_at FROM updates ORDER BY created_at DESC, id DESC";
...
// Admin Updates - Get single
if (isset($_POST['updates_get_admin'])) {
    global $pdo;
    $conn = $pdo->open();
    $id = isset($_POST['id']) ? intval($_POST['id']) : 0;
    $stmt = $conn->prepare("SELECT id, title_en, content_en, title_swg, content_swg, visible FROM updates WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch();
    echo json_encode(['update' => $row]);
}
// Admin Updates - Create
if (isset($_POST['updates_create_admin'])) {
    global $pdo;
    $conn = $pdo->open();
    $title_en = isset($_POST['title_en']) ? $_POST['title_en'] : null;
    $content_en = isset($_POST['content_en']) ? $_POST['content_en'] : null;
    $title_swg = isset($_POST['title_swg']) ? $_POST['title_swg'] : null;
    $content_swg = isset($_POST['content_swg']) ? $_POST['content_swg'] : null;
    $visible = isset($_POST['visible']) ? intval($_POST['visible']) : 1;
    $stmt = $conn->prepare("INSERT INTO updates (title_en, content_en, title_swg, content_swg, visible, created_at) VALUES (?,?,?,?,?,NOW())");
    $stmt->execute([$title_en, $content_en, $title_swg, $content_swg, $visible]);
    echo json_encode(['ok' => true, 'id' => $conn->lastInsertId()]);
}
// Admin Updates - Update
if (isset($_POST['updates_update_admin'])) {
    global $pdo;
    $conn = $pdo->open();
    $id = isset($_POST['id']) ? intval($_POST['id']) : 0;
    $title_en = isset($_POST['title_en']) ? $_POST['title_en'] : null;
    ...
    $stmt = $conn->prepare("UPDATE updates SET title_en = ?, content_en = ?, title_swg = ?, content_swg = ?, visible = ?, updated_at = NOW() WHERE id = ?");
    $stmt->execute([$title_en, $content_en, $title_swg, $content_swg, $visible, $id]);
    echo json_encode(['ok' => true]);
}
// Admin Updates - Toggle
if (isset($_POST['updates_toggle_admin'])) {
    global $pdo;
    $conn = $pdo->open();
    $id = isset($_POST['id']) ? intval($_POST['id']) : 0;
    $conn->exec("UPDATE updates SET visible = IF(visible=1,0,1), updated_at = NOW() WHERE id = {$id}");
    echo json_encode(['ok' => true]);
}
// Admin Updates - Delete
if (isset($_POST['updates_delete_admin'])) {
    global $pdo;
    $conn = $pdo->open();
    $id = isset($_POST['id']) ? intval($_POST['id']) : 0;
...
if (isset($_POST['background_check'])) {
    ...
    // Check if all background checks are completed (all are 0)

    $economy = isset($candidate->economy) ? (int)$candidate->economy : -1;
    $criminal_record = isset($candidate->criminal_record) ? (int)$candidate->criminal_record : -1;
    $social = isset($candidate->social) ? (int)$candidate->social : -1;
    $allCompleted = (($economy == 0 || $economy == 1) && ($criminal_record == 0 || $criminal_record == 1) && ($social == 0 || $social == 1));
    $response = [
                ...
                foreach ($columns_arr as $k => $column) {
                    foreach ($column as $key => $val) {
                        ...
                        if ($val == 'invoice_sent') {
                            ...
                            } elseif (! empty($candidate['invoice_sent'])) {
                                // $invoice_sent = "✔️";
...
if (isset($_POST['formHtml']) || (isset($_POST['formHtml_level1']) && isset($_POST['formHtml_level2']))) {
    global $pdo;
    $conn = $pdo->open();
    $lang = isset($_POST['lang']) ? $_POST['lang'] : 'sv';
    if (isset($_POST['formHtml_level1']) && isset($_POST['formHtml_level2'])) {
        $report_data = json_encode(['1' => $_POST['formHtml_level1'], '2' => $_POST['formHtml_level2']]);
    } else {
        $report_data = $_POST['formHtml'];
    }
    ...
    if (! empty($report_html)) {
        ...
        $stmt = $conn->prepare($query);
        $stmt->execute([$report_data, $_POST['id'], $lang]);
    } else {
        $query = "INSERT INTO reports_html (candidate_id, report_data, lang) VALUES (?,?,?)";
        $stmt = $conn->prepare($query);
        $stmt->execute([$_POST['id'], $report_data, $lang]);
    }
}
...
if (isset($_POST['comp_report'])) {
    ...
    if (isset($_POST['compformHtml_level1']) && isset($_POST['compformHtml_level2'])) {
        $comp_report_data = json_encode(['1' => $_POST['compformHtml_level1'], '2' => $_POST['compformHtml_level2']]);
    } else {
        $comp_report_data = $_POST['compformHtml'];
    }
    ...
    if (! empty($report_html)) {
        ...
        $stmt->execute([$comp_report_data, $meta_info_json, $customer, $lang, $interview_id]);
    } else {
        ...
        $meta_info_json = json_encode($meta_info);
        $stmt->execute([$customer, $comp_report_data, $interview_id, $lang, $meta_info_json]);
    }
}
...
if (isset($_POST['cus_report'])) {
    if (isset($_POST['customer']) && ! empty($_POST['customer'])) {
        ...
        $interview_id = $_POST['interview_id'];
        if (isset($_POST['compformHtml_level1']) && isset($_POST['compformHtml_level2'])) {
            $cus_report_data = json_encode(['1' => $_POST['compformHtml_level1'], '2' => $_POST['compformHtml_level2']]);
        } else {
            $cus_report_data = $_POST['compformHtml'];
        }
        ...
        if (! empty($report_html)) {
            ...
            $meta_info_json = json_encode($meta_info);
            $stmt->execute([$cus_report_data, $meta_info_json, $customer, $lang, $interview_id]);
            ...
            if (! empty($child_report_html)) {
                foreach ($child_report_html as $child_report) {
                    ...
                    if (! empty($customer_check)) {
                        $query = "UPDATE customer_reports_html SET report_data = ?, meta_info = ? WHERE cus_id = ? AND lang = ? AND interview_id = ?";
                        $stmt = $conn->prepare($query);
                        $stmt->execute([$cus_report_data, $meta_info_json, $child_report->id, $lang, $interview_id]);
                    } else {
                        ...
                        $meta_info_json = json_encode($meta_info);
                        $stmt->execute([$child_report->id, $cus_report_data, $interview_id, $lang, $meta_info_json]);
                    }
                }
            }
        } else {
            ...
            $meta_info_json = json_encode($meta_info);
            $stmt->execute([$customer, $cus_report_data, $interview_id, $lang, $meta_info_json]);
            $query = "SELECT id FROM customers WHERE parent_id = ?";
            $stmt = $conn->prepare($query);
            $stmt->execute([$customer]);
            $child_report_html = $stmt->fetchAll();
