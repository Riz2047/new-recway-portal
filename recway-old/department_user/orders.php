...
if (!isset($user_allowed_permissions['View-order']) && empty($user_allowed_permissions['View-order'])) {
    redirect('index.php');
}
...
if (isset($_GET['status'])) {
    $query = "SELECT * FROM candidates WHERE cus_id = ? AND dep_id = ? AND status = {$_GET['status']} {$query_st} AND expired = 0";
    $stmt = $conn->prepare($query);
    $stmt->execute([$_SESSION['department_user']->dep_cus_id, $_SESSION['department_user']->dep_id]);
    $candidates = $stmt->fetchAll();
} else {
    $query = "SELECT * FROM candidates WHERE cus_id = ? AND dep_id = ? {$query_st} AND expired = 0";
    $stmt = $conn->prepare($query);
    $stmt->execute([$_SESSION['department_user']->dep_cus_id, $_SESSION['department_user']->dep_id]);
    $candidates = $stmt->fetchAll();
}
