...
$permissions = findallByQuery("SELECT * FROM user_permissions JOIN user_allowed_permissions ON user_permissions.id = user_allowed_permissions.per_id WHERE user_allowed_permissions.user_id = '{$_SESSION['department_user']->dep_user_id}' AND user_allowed_permissions.user_type = 1");
...
$query = 'SELECT * FROM candidates WHERE cus_id = ? AND dep_id = ? AND expired = 0';
$stmt = $conn->prepare($query);
$stmt->execute([$_SESSION['department_user']->dep_cus_id, $_SESSION['department_user']->dep_id]);
...
$query = "SELECT * FROM customer_services WHERE customer_services.cus_id={$_SESSION['department_user']->dep_cus_id}";
$stmt = $conn->prepare($query);
$stmt->execute();
...
$query = "SELECT * FROM department_users WHERE dep_user_id={$_SESSION['department_user']->dep_user_id} AND dep_user_trash = 0";
$stmt = $conn->prepare($query);
$stmt->execute();
$department_user = $stmt->fetch();
?>
        ...
        <aside class="sidebar">
            ...
            <div class="profile-information mt-1">
                <div class="candidate-profile">
                    <?php $cusName = explode(" ", $department_user->dep_user_name); ?>
                    <?php
                    if (count($cusName) > 1) {
                ...
                <h1 class="f-18 w-700 mb-0 pb-0 mt-2"><?php echo $department_user->dep_user_name ?></h1>
                <p class="f-16 text-grey w-500"><?php echo $department_user->dep_user_email ?></p>
            </div>
                                ...
                                <div class="dropdown-container">
                                    ...
                                    <?php if (!empty($statuses)) : ?>
                                        <?php foreach ($statuses as $status) : ?>
                                            <a href="orders.php?status=<?php echo $status->sID ?>" class="d-flex f-14 w-600 text-grey mb-3 no-decoration dropdown-container-link"><?php echo $status->status ?> <?php echo isset($countStatus) && isset($countStatus[$status->sID]) ? '<div class="notification-number ms-2 mt-1">' . $countStatus[$status->sID] . '</div>' : '' ?> </a>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="d-flex justify-content-between align-items-lg-end align-items-center   w-100">
                                <div>
                                    <h1 class="f-30 w-700 font-secondary m-0 p-0">User Dashboard</h1>
                                </div>
                                <div class="d-flex align-items-center">
