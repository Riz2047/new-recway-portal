...
$query = "SELECT * FROM department_services WHERE dep_id={$_SESSION['department_user']->dep_id}";
$stmt = $conn->prepare($query);
$stmt->execute();
...
if (!empty($interviews) && !empty($customer_services)) {
    ...
    array_walk($interviews, function ($interview, $key) {
        ...
        if (in_array($interview->id, array_column($customer_services, "dep_service_id"))) {
