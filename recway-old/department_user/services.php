...
$query = "SELECT * FROM department_services WHERE department_services.dep_id={$_SESSION['department_user']->dep_id}";
$stmt = $conn->prepare($query);
$stmt->execute();
...
foreach ($customer_services as $service) {
  ...
  $stmt = $conn->prepare($query);
  $stmt->execute([$service->dep_service_id]);
...
$candidates = findAllByQuery("SELECT * FROM candidates WHERE cus_id = {$_SESSION['department_user']->dep_cus_id} AND dep_id = {$_SESSION['department_user']->dep_id} AND expired = 0 ORDER BY created DESC LIMIT 4");
?>
          ...
          <div class="row p-0">
            ...
            <div class="col-lg-12">
              <div class="grey-box mt-2 ">
                ...
                <div class="d-flex justify-content-between align-items-center ">
                  <h1 class="f-24 w-600 text-black mb-0 pb-0"> <a href="mailto:info@recway.nu" class="no-decoration text-black f-16 ">info@recway.nu</a></h1>
                </div>
