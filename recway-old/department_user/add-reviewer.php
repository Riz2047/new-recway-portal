...
if (!isset($user_allowed_permissions['Create-reviewer']) && empty($user_allowed_permissions['Create-reviewer'])) {
    redirect('index.php');
 }
...
if (isset($_POST['add'])) {
    ...
    $reviewer = findByQuery("SELECT * FROM reviewers WHERE cus_id = {$_SESSION['department_user']->dep_cus_id} AND email = '{$email}'");

    if (!empty($reviewer)) {
        $msg = "<p class='text-danger'>This email already exists!</p>";
    } else {
        $hashedPassword = password_hash($password, PASSWORD_BCRYPT);
        insert("reviewers", ["cus_id" => $_SESSION['department_user']->dep_cus_id, "email" => $email, "password" => $hashedPassword]);

        $body = "<p>You are added as a reviewer by {$_SESSION['department_user']->dep_user_name}. Please use following credentials to login.</p>";
        $body .= "<br>";
