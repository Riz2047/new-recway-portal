        ...
        foreach ($can_id as $k => $canid) {
            ...
            if ($status->variable == "booked" || $status->variable == "booked_msg_follow") {
                ...
                if (isset($_SESSION['staff']->id) && !empty($_SESSION['staff']->id)) {
                    ...
                    $stmt = $conn->prepare($query);
                    $res = $stmt->execute([$_SESSION['staff']->id, " changed the status of <a href='invoice.php?id={$canid}'>{$orderID[$k]}</a> to {$status->status}"]);
                }
            ...
            if (!empty($res)) {
                ...
                if (!empty($msg)) {
                    ...
                    if ($dayOfWeek >= 1 && $dayOfWeek <= 5 && $currentTime > '08:00:00' && $currentTime < '18:00:00') {
                        $body = replace($msg, $cus_name[$k], $can_name[$k] . " " . $candidate->surname, $cus_company[$k], $interview[$k], !empty($staff) ? $staff->name : '', '', '', $status->status, $date, $orderID[$k], !empty($_POST['booked'][$k]) ? $_POST['booked'][$k] : $date, !empty($staff) ? $staff->email : '', $comment, $candidate->vasc_id, $service->title, !empty($place) ? $place->name : '');
