...
$(document).ready(function () {
        ...
        table = $tbl.DataTable({
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        ...
                        add('delivery_date_from', $('#delivery_date_from').val() || '');
                        add('delivery_date_to', $('#delivery_date_to').val() || '');
            ...
            ajax: {
                ...
                data: function (d) {
                    ...
                    d.delivery_date_from = $('#delivery_date_from').val() || '';
                    d.delivery_date_to = $('#delivery_date_to').val() || '';
            ...
            drawCallback: function (settings) {
                ...
                $('.bg-check-alert-icon').off('mouseenter.bgcheck mouseleave.bgcheck');                // Initialize tooltips for background check alert icons
                $('.bg-check-alert-icon').each(function () {
                    var $icon = $(this);
                    var orderId = $icon.attr('data-order-id');                    if (!orderId) {
                        return;
                    }                    // Destroy existing tooltip if any
                    var existingTooltip = bootstrap.Tooltip.getInstance(this);
                    if (existingTooltip) {
                        existingTooltip.dispose();
                    }                    // Create tooltip with manual trigger (we'll control it ourselves)
                    var tooltip = new bootstrap.Tooltip(this, {
                        html: true,
                        placement: 'top',
                        title: '<div style="text-align: center; padding: 5px;">Loading...</div>',
                        trigger: 'manual' // Manual trigger - we control when it shows
                    });                    // Store order_id and tooltip instance
                    $icon.data('order-id', orderId);
                    $icon.data('tooltip-instance', tooltip);
                    $icon.removeData('tooltip-loading');
                    $icon.removeData('ajax-request');
                });                // Use mouseenter directly instead of Bootstrap tooltip events
                $('.bg-check-alert-icon').on('mouseenter.bgcheck', function (e) {
                    e.stopPropagation();                    var $icon = $(this);
                    var orderId = $icon.attr('data-order-id') || $icon.data('order-id');
                    var iconElement = this;                    if (!orderId) {
                        return;
                    }                    // Get tooltip instance
                    var tooltip = $icon.data('tooltip-instance') || bootstrap.Tooltip.getInstance(iconElement);
                    if (!tooltip) {
                        tooltip = new bootstrap.Tooltip(iconElement, {
                            html: true,
                            placement: 'top',
                            title: '<div style="text-align: center; padding: 5px;">Loading...</div>',
                            trigger: 'manual',
                            boundary: 'viewport',
                            delay: { show: 0, hide: 0 } // No delay
                        });
                        $icon.data('tooltip-instance', tooltip);
                    }                    // Prevent any auto-hide behavior
                    $(iconElement).off('hide.bs.tooltip').on('hide.bs.tooltip', function (e) {
                        // Only hide if mouse is not over icon or tooltip
                        if ($icon.is(':hover') || $('.tooltip').is(':hover')) {
                            e.preventDefault();
                            return false;
                        }
                    });                    // Show tooltip with loading message immediately
                    tooltip.setContent({ '.tooltip-inner': '<div style="text-align: center; padding: 5px;">Loading...</div>' });
                    tooltip.show();                    // Check if AJAX request is already in progress
                    if ($icon.data('tooltip-loading')) {
                        return; // Already fetching, don't make duplicate request
                    }                    // Mark as loading
                    $icon.data('tooltip-loading', true);                    // Make AJAX request
                    var ajaxRequest = $.ajax({
                        url: '../includes/pages.php',
                        type: 'POST',
                        data: {
                            action: 'get_bg_check_status',
                            order_id: orderId
                        },
                        dataType: 'json',
                        success: function (response) {
                            // Only update if mouse is still over the icon
                            if (!$icon.is(':hover')) {
                                $icon.removeData('tooltip-loading');
                                return;
                            }                            $icon.removeData('tooltip-loading');                            if (response.success && response.tooltip_html) {
                                var currentTooltip = $icon.data('tooltip-instance') || bootstrap.Tooltip.getInstance(iconElement);
                                if (currentTooltip) {
                                    currentTooltip.setContent({ '.tooltip-inner': response.tooltip_html });
                                ...
                                var errorMsg = response.error || 'Failed to load background check status';
                                var currentTooltip = $icon.data('tooltip-instance') || bootstrap.Tooltip.getInstance(iconElement);
                                if (currentTooltip) {
                                    currentTooltip.setContent({ '.tooltip-inner': '<div style="color: #ff4444; padding: 5px;">' + errorMsg + '</div>' });
                                    if ($icon.is(':hover')) {
                                        currentTooltip.show();
                                    }
                                }
                        ...
                        error: function (xhr, status, error) {
                            if (status !== 'abort') {
                                $icon.removeData('tooltip-loading');
                                // Only show error if mouse is still over
                                if ($icon.is(':hover')) {
                                    var currentTooltip = $icon.data('tooltip-instance') || bootstrap.Tooltip.getInstance(iconElement);
                                    if (currentTooltip) {
                                        currentTooltip.setContent({ '.tooltip-inner': '<div style="color: #ff4444; padding: 5px;">Error loading data</div>' });
                                        currentTooltip.show();
                                    }
                                }
                            }
                    ...
                    $icon.data('ajax-request', ajaxRequest);
                }).on('mouseleave.bgcheck', function () {
                    var $icon = $(this);
                    var tooltip = $icon.data('tooltip-instance') || bootstrap.Tooltip.getInstance(this);                    // Hide tooltip when mouse leaves
                    if (tooltip) {
                        tooltip.hide();
                    }                    // Abort any pending AJAX request
                    var ajaxRequest = $icon.data('ajax-request');
                    if (ajaxRequest && ajaxRequest.readyState !== 4) {
                        ajaxRequest.abort();
                    }                    $icon.removeData('ajax-request');
                    $icon.removeData('tooltip-loading');
                });                // Prevent tooltip from hiding when mouse moves over tooltip itself
                $(document).on('mouseenter', '.tooltip', function () {
                    ...
                    $('.bg-check-alert-icon').each(function () {
                        if (!$(this).is(':hover')) {
                            var tooltip = $(this).data('tooltip-instance') || bootstrap.Tooltip.getInstance(this);
                            if (tooltip) {
                                tooltip.hide();
                            }
                        }
        ...
        table.on('draw.dt', function(){ updateSelectAllState(); });
    } else if ($('#dataTable').attr('data-table') === 'custom_messages') {
			// Custom Messages DataTable
			table = $('#dataTable').DataTable({
					...
					ajax: {
							...
							type: 'POST',
							data: function (d) {
									d.action = 'get_custom_messages_data';
									return d;
							},
							complete: function () {
									$('.dropdownBtn').each(function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (that.closest('tr').length) {
                ...
                // Hide red blinking icon if all checks are completed
                if (response.allCompleted === true) {
                    that.closest('tr').find('.bg-check-alert-icon').fadeOut(300, function() {
                        $(this).remove();
                    });
                    that.closest('tr').next().find('.bg-check-alert-icon').fadeOut(300, function() {
                        $(this).remove();
                    });
                }
            }
            if (that.closest(".record-main").length) {
                ...
                // Hide red blinking icon if all checks are completed
                if (response.allCompleted === true) {
                    that.closest('.record-main').find('.bg-check-alert-icon').fadeOut(300, function() {
                        $(this).remove();
                    });
                }
            }
            
            // Remove candidate from notification dropdown if all checks are completed
            if (response.allCompleted === true && response.candidate && response.candidate.id) {
                $('#bg-check-' + response.candidate.id).fadeOut(300, function() {
                    $(this).remove();
...
function updateNotificationBadge() {
    var notificationCount = $('#bg-check-menus li').not(':has(.text-muted)').length;
    var $badge = $('.notification-badge-count');
    var $notificationWrapper = $badge.closest('.profile-img');
    
    if (notificationCount > 0) {
        $badge.text(notificationCount > 99 ? '99+' : notificationCount);
        if ($badge.length === 0) {
            // Re-add badge if it was removed
            $notificationWrapper.append('<span class="notification-badge-count" style="position: absolute; top: 5px; right: 5px; background: #ff4757; color: white; border-radius: 50%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 600; border: 2px solid #14256a;">' + (notificationCount > 99 ? '99+' : notificationCount) + '</span>');
        }
    } else {
        // Hide badge and notification icon if no pending checks
        $badge.fadeOut(300, function() {
            $(this).remove();
        });
        $notificationWrapper.removeClass('has-dot');
        
        // Update notification list to show empty state
        if ($('#bg-check-menus li').length === 0 || ($('#bg-check-menus li').length === 1 && $('#bg-check-menus li').has('.text-muted').length > 0)) {
            $('#bg-check-menus').html('<li style="padding: 20px; text-align: center;"><p class="text-muted m-0" style="font-size: 14px;"><i class="bi bi-check-circle" style="font-size: 24px; color: #4be69d; display: block; margin-bottom: 10px;"></i>All background checks are complete</p></li>');
        }
    }
}
// Bootstrap Icons
...
document.addEventListener('hide.bs.dropdown', function (event) {
	...
	if (menu && dropdown) dropdown.appendChild(menu);
});