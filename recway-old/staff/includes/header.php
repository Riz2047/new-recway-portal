...
function getStatusCountOfStaff($statusId, $candidates_addition_query = '', $queryParams = [])
{
    global $conn;
    $serviceClause = '';
    $params = [$statusId];
...
function getneworderscount($serviceCategoryId, $candidates_addition_query = null)
{
    ...
    if (empty($statusId)) {
        // Fallback to the earliest status in this category
        $stmt = $conn->prepare("SELECT id FROM statuses WHERE status_type = ? ORDER BY id ASC LIMIT 1");
        $stmt->execute([$serviceCategoryId]);
        $statusId = $stmt->fetchColumn();
        if (empty($statusId)) {
            return 0;
    ...
    // Count candidates by resolved status id and current scope
    $countSql = 'SELECT COUNT(candidates.id) FROM candidates WHERE candidates.status = ? AND expired = 0' . $candidates_addition_query;
