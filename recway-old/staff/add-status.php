...
if (isset($_POST['add'])) {
    ...
    $query = 'INSERT INTO statuses(variable, status, color, status_detail, status_icon,status_type) VALUES(?,?,?,?,?)';
    $stmt = $conn->prepare($query);
    $res = $stmt->execute([$variable, $status, $color, $status_detail, $icon, $status_type]);
    if (!empty($res)) {
