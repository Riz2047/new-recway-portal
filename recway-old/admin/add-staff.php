...
if(isset($_POST['add_staff'])) {
    ...
    $query = 'INSERT INTO staff (name,email,password,phone) VALUES (?,?,?,?)';
    $stmt = $conn->prepare($query);
    $res = $stmt->execute([$name, $email, $crypt_pass, $phone]);
    if(!empty($res)) {
        ...
        $message = "<p class='alert alert-success'>Staff added successfully!</p>";
        redirect('staff.php');
    } else {
        $message = "<p class='alert alert-danger'>Could not add staff!</p>";
    }
}
                            ...
                            <form action="" method="post">
                                <div class="row p-0 m-0">
                                    ...
                                    <div class="col-lg-12 ps-0">
                                        <button type="submit" name="add_staff" class="btn-fill w-100 mt-4"><a>Save</a></button>
                                    </div>
                                </div>
                            </form>
                        </div>
