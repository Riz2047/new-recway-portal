                  ...
                  <div class="form-tag mb-2">Select emails which you want to receive</div>
                    ...
                    <p class="fw-bold mt-2 mb-0"><?php echo $service->name ?></p>
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-3">
                                <label><input <?php echo isEmailAllowed($_SESSION['customer']->id,$status->sID) ? 'checked' : '' ?> type="checkbox" class="emails-status" value="<?php echo $status->sID ?>" data-cus-id="<?php echo $_SESSION['customer']->id ?>"> <?php echo $status->status ?></label>
                            </div>
                            <?php endforeach; ?>
                            <?php endif; ?>
                        </div>
...
<script>
    $(".emails-status").click(function () {
        $.ajax({
            ...
            method: "post",
            data: {email_status: "true", status_id: $(this).val(), cus_id: $(this).data("cus-id"), allowed: $(this).is(":checked")},
